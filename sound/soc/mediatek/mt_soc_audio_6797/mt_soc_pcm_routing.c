...
#ifdef MEIZU_M80
static const char const *Audio_Debug_Setting[] = { "Off", "Speaker", "Headphone", "Earpiece" };
#else
				  ...
				  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (output_device == 1) {			//Speaker
		EnableSineGen(Soc_Aud_InterConnectionOutput_O00,
				Soc_Aud_MemIF_Direction_DIRECTION_OUTPUT, true);
	} else if (output_device == 2) {	//Headphone
		EnableSineGen(Soc_Aud_InterConnectionOutput_O28,
				Soc_Aud_MemIF_Direction_DIRECTION_OUTPUT, true);
	} else {							//Earpiece
		EnableSineGen(Soc_Aud_InterConnectionOutput_O03,
