...
#ifdef MEIZU_M80
	if ((samplerate == 0 && ucontrol->value.integer.value[0] == 0)
		 || (samplerate && ucontrol->value.integer.value[0])) {
		pr_err("%s: value(%ld), but pre samplerate is %d\n",
			__func__, ucontrol->value.integer.value[0], samplerate);
		return -1;
	}
...
static int mi2s0_clk_sel_control = CLK_I2S3;	//default i2s3
static int mi2s0_clk_control = 0;
static int mi2s1_sidegen_control = 0;
static int mi2s1_hdoutput_control = 0;
static const char const *i2s0_clk_sel[] = { "i2s0", "i2s3" };
static const char const *i2s1_HD_output[] = { "Off", "On" };
static const char const *i2s1_SIDEGEN[] = {
	"Off", "On192000", "On176400", "On96000", "On88200", "On64000",
	"On48000", "On44100", "On32000", "On24000", "On16000", "On8000"
};

static const struct soc_enum Audio_i2s1_Enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(i2s1_HD_output), i2s1_HD_output),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(i2s1_SIDEGEN), i2s1_SIDEGEN),
};

static const struct soc_enum Audio_i2s0_clk_Enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(i2s0_clk_sel), i2s0_clk_sel),
};

static struct SampleRateToMixerName {
    int samplerate;
    const char *name;
} gSampleRateToMixerName[] = {
    {192000, "On192000"},
    {176400, "On176400"},
    {96000,  "On96000"},
    {88200,  "On88200"},
...
static int SideGenNameToSampleRate(const char *name)
{
    int i = 0;

    for (i = 0; i < sizeof(gSampleRateToMixerName)/sizeof(gSampleRateToMixerName[0]); i++) {
        if (!strncmp(gSampleRateToMixerName[i].name, name, strlen(name))) {
            return gSampleRateToMixerName[i].samplerate;
        }
    }
    return -1;
}

static int Audio_i2s0_Clk_sel_Get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("mi2s0_clk_sel_control = %d\n", mi2s0_clk_sel_control);
	ucontrol->value.integer.value[0] = mi2s0_clk_sel_control;
	return 0;
}

static int Audio_i2s0_Clk_sel_Set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: value(%ld)\n", __func__, ucontrol->value.integer.value[0]);

	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(i2s0_clk_sel)) {
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}

	mi2s0_clk_sel_control = ucontrol->value.integer.value[0];

	if (mi2s0_clk_sel_control == CLK_I2S3) {//i2s3
		//mt_set_gpio_mode(69 | 0x80000000, GPIO_MODE_02);
		//mt_set_gpio_mode(70 | 0x80000000, GPIO_MODE_02);
		AudDrv_GPIO_SMARTPA_Select(2);
	} else {//i2s0
		//mt_set_gpio_mode(69 | 0x80000000, GPIO_MODE_01);
		//mt_set_gpio_mode(70 | 0x80000000, GPIO_MODE_01);
		AudDrv_GPIO_SMARTPA_Select(1);
	}

	return 0;
}

static int Audio_i2s0_Clk_Get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("mi2s0_clk_control = %d\n", mi2s0_clk_control);
	ucontrol->value.integer.value[0] = mi2s0_clk_control;
	return 0;
}

static int Audio_i2s0_Clk_Set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	uint32 u32AudioI2S = 0;	/* REG448 = 0, REG44C = 0; */
	uint32 Audio_I2S_Dac = 0;
	...
	pr_debug("%s: value(%ld), samplerate(%d)\n", __func__, ucontrol->value.integer.value[0], samplerate);
	...
	mi2s0_clk_control = ucontrol->value.integer.value[0];
	if (mi2s0_clk_control == 1) {
		samplerate = 48000;
	} else if (mi2s0_clk_control == 2) {
		samplerate = 44100;
	} else if (mi2s0_clk_control == 3) {
		samplerate = 32000;
	} else if (mi2s0_clk_control == 4) {
		samplerate = 16000;
	} else if (mi2s0_clk_control == 5) {
		samplerate = 8000;
	}

	if (mi2s0_clk_control) {
		AudDrv_Clk_On();
		EnableALLbySampleRate(samplerate);
...
static int Audio_i2s1_hdoutput_Get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("mi2s1_hdoutput_control = %d\n", mi2s1_hdoutput_control);
	ucontrol->value.integer.value[0] = mi2s1_hdoutput_control;
	return 0;
}

static int Audio_i2s1_hdoutput_Set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("+%s()\n", __func__);
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(i2s1_HD_output)) {
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}

	mi2s1_hdoutput_control = ucontrol->value.integer.value[0];
	return 0;
}

static int Audio_i2s1_SideGen_Get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	pr_info("mi2s1_sidegen_control = %d\n", mi2s1_sidegen_control);
	ucontrol->value.integer.value[0] = mi2s1_sidegen_control;
	return 0;
}

static int Audio_i2s1_SideGen_Set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	static uint32 samplerate = 0;
	bool mI2SWLen = Soc_Aud_I2S_WLEN_WLEN_32BITS;

	pr_info("%s: value(%ld), samplerate(%d)\n", __func__, ucontrol->value.integer.value[0], samplerate);


	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(i2s1_SIDEGEN)) {
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
	...
	mi2s1_sidegen_control = ucontrol->value.integer.value[0];

	if (mi2s1_sidegen_control) {
		samplerate = SideGenNameToSampleRate(i2s1_SIDEGEN[mi2s1_sidegen_control]);
		pr_info("%s: samplerate(%d), mi2s1_sidegen_control(%d)\n", __func__, samplerate, mi2s1_sidegen_control);

		mI2SWLen = Soc_Aud_I2S_WLEN_WLEN_32BITS;
		if (samplerate <= 16000) {	//Speech Mode
			mI2SWLen = Soc_Aud_I2S_WLEN_WLEN_32BITS;		//Soc_Aud_I2S_WLEN_WLEN_16BITS
			/* here start digital part */
			SetConnection(Soc_Aud_InterCon_Connection,
				      Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O28);
			SetConnection(Soc_Aud_InterCon_Connection,
				      Soc_Aud_InterConnectionInput_I14, Soc_Aud_InterConnectionOutput_O29);
		}
		...
		Afe_Set_Reg(AFE_I2S_CON1, 0, 0x1);
		udelay(20);

		SetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_DAC, true);
		SetI2SDacOut(samplerate, mi2s1_hdoutput_control, mI2SWLen);
...
int mt_soc_smartpa_i2s_clk_enable(bool enable)
{
	static uint32 samplerate = 0;
	static bool is_enable = 0;
	uint32 u32AudioI2S = 0;
	uint32 Audio_I2S_Dac = 0;

	if (is_enable == enable) {
		pr_warn("%s: is_enable(%d)=enable\n", __func__, is_enable);
		return 0;
	}
...
EXPORT_SYMBOL(mt_soc_smartpa_i2s_clk_enable);
#endif
...
#ifdef MEIZU_M80
	SOC_ENUM_EXT("Audio_i2s0_Clk_Sel", Audio_i2s0_clk_Enum[0],
		Audio_i2s0_Clk_sel_Get, Audio_i2s0_Clk_sel_Set),
	SOC_ENUM_EXT("Audio_i2s0_Clk_Switch",
		     Audio_i2s0_Enum[0], Audio_i2s0_Clk_Get, Audio_i2s0_Clk_Set),
	SOC_ENUM_EXT("Audio_i2s1_hd_Switch",
		     Audio_i2s1_Enum[0], Audio_i2s1_hdoutput_Get, Audio_i2s1_hdoutput_Set),
	SOC_ENUM_EXT("Audio_i2s1_SideGen_Switch", Audio_i2s1_Enum[1],
		Audio_i2s1_SideGen_Get, Audio_i2s1_SideGen_Set),
#endif
};
