...
#ifdef MEIZU_M80
//force low-jitter
static int mdl2_hdoutput_control = true;
#endif
...
#ifdef MEIZU_M80
		if (mdl2_hdoutput_control == true) {
			pr_warn("%s mdl2_hdoutput_control == %d\n", __func__,
			       mdl2_hdoutput_control);
			/* here to open APLL */
...
static int mtk_pcm_dl2_prepare(struct snd_pcm_substream *substream)
{
	...
	if (mPrepareDone == false) {
		...
		if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_DAC) == false) {
			...
			SetI2SDacOut(substream->runtime->rate, mdl2_hdoutput_control, mI2SWLen);
...
//fastmixer playback for smart pa
#ifdef MEIZU_M80
	SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I07,
		      Soc_Aud_InterConnectionOutput_O00);
	SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I08,
