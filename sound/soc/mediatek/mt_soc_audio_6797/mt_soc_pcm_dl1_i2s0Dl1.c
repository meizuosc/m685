...
static int Audio_Loopback_Get(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.enumerated.item[0] = loopback_enable;
    pr_err("%s(), ucontrol->value.enumerated.item[0]; %d, loopback_enable %d\n",
		__func__, ucontrol->value.enumerated.item[0], loopback_enable);

    return 0;
}

static int Audio_Loopback_Set(struct snd_kcontrol *kcontrol,
                              struct snd_ctl_elem_value *ucontrol)
{
...
#ifdef MEIZU_M80
	SOC_SINGLE_BOOL_EXT("Audio_Mic_Sealing_Test_Switch", 0,
			Audio_Loopback_Get, Audio_Loopback_Set),
#endif
};
...
static int mtk_pcm_I2S0dl1_start(struct snd_pcm_substream *substream)
{
	...
	SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I06,
		      Soc_Aud_InterConnectionOutput_O01);
#ifdef MEIZU_M95
	if (loopback_enable) { /* DL1_data -> O3 (LEFT channel); I03 & I04 -> O04 */
		SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I05,
			      Soc_Aud_InterConnectionOutput_O03);
		SetConnection(Soc_Aud_InterCon_Connection, Soc_Aud_InterConnectionInput_I06,
			      Soc_Aud_InterConnectionOutput_O03);
