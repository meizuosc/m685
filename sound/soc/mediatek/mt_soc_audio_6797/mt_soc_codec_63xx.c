...
static void Voice_Audio_Amp_Change(bool enable);
...
/* Left Channel -> earpiece; Right Channel -> Headphone Right Channel */
static void Voice_Audio_Amp_Change(bool enable)
{
	pr_warn("func %s, enable %d\n", __func__, enable);
	if (enable) {
		if (GetDLStatus() == false)
			TurnOnDacPower();
		...
		Ana_Set_Reg(AUDDEC_ANA_CON9, 0xA255, 0x0200);
		/* Enable AUD_CLK */
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0xE09F, 0xffff); //xxxxxxx
		/* Enable Audio DAC  */
		...
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0xF10F, 0xffff); //xxxxx
		/* Switch HP MUX to audio DAC */
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0xF15F, 0xffff); //xxxxx
		/* Enable HPR / Voice */
		Ana_Set_Reg(AUDDEC_ANA_CON1, 0x2100, 0xffff);
		/* Disable pre-charge buffer */
