...
#define TFA98XX_I2C_DEV_SUPPORT

#define EVENT_TYPE_TAPDET_VALUE          REL_MISC
...
static int tfa98xx_tapdet_profile = -1;
...
enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int *vstep)
{
	...
	dev_info(tfa98xx->codec->dev, "tfa98xx_tfa_start\n");
...
*/
	...
	input_set_capability(input, EV_REL, EVENT_TYPE_TAPDET_VALUE);

	input->open = tfa98xx_input_open;
	input->close = tfa98xx_input_close;

	input->name = "tapdet";		//"tfa98xx-tapdetect"
...
void __tfa98xx_inputdev_check_register(struct tfa98xx *tfa98xx, bool unregister)
{
	...
	for (i = 0; i < tfaContMaxProfile(tfa98xx->handle); i++) {
		if (strstr(tfaContProfileName(tfa98xx->handle, i), "tap")) {	//.tap
			tap_profile = true;
...
{
	int profile = kcontrol->private_value;
...
{
	int profile = kcontrol->private_value;// tfaContGetCurrentProfile();
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tfa98xx->status == TFA98XX_STANDBY
		...
		pr_info("Current status is %d\n", tfa98xx->status);
		return 1;
	}

	tfa98xx_profile = new_profile;
	tfa98xx_vsteps[0] = tfa98xx_prof_vsteps[new_profile];
	tfa98xx_vsteps[1] = tfa98xx_prof_vsteps[new_profile];
	...
	if (ready) {
		err = tfa98xx_tfa_start(tfa98xx, new_profile, tfa98xx_vsteps);
		if (err) {
...
//added by ywl
int tfa98xx_set_profile_force(struct tfa98xx *tfa98xx, int new_profile)
{
	int err;
	int ready = 0;
...
int tfa98xx_set_tapdet_profile_enable(struct tfa98xx *tfa98xx, bool enable)
{
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_err("DSP not loaded(no FW: %d)\n", tfa98xx->dsp_fw_state);
		return -1;
	}

	if (tfa98xx_tapdet_profile < 0) {
		pr_err("tfa98xx_tapdet_profile = %d\n", tfa98xx_tapdet_profile);
		return -1;
	}

	pr_info("enable(%d), tfa98xx_tapdet_profile(%d), tfa98xx_profile(%d)\n",
			enable, tfa98xx_tapdet_profile, tfa98xx_profile);

	if (enable) {
		//workaround fix the issue of continues open tapdet will be failed
		//if (tfa98xx_tapdet_profile == tfa98xx_profile) {
		//	pr_info("pre profile is tapdet\n");
		//	tfa98xx_set_profile_force(tfa98xx, 0);
		//}
		tfa98xx_set_profile_force(tfa98xx, tfa98xx_tapdet_profile);
	} else {
		if (tfa98xx_tapdet_profile == tfa98xx_profile) {
			pr_info("stop dsp\n");
			//stop dsp
...
static int is_profile_in_list(char *profile, int len) 
{
    struct tfa98xx_baseprofile *bprof;    
    list_for_each_entry(bprof, &profile_list, list) {
        //if (0 == strncmp(bprof->basename, profile, len)) {	//modified by ywl
        if (0 == strcmp(bprof->basename, profile)) {
            return 1;
        }
    }
    return 0;
...
int is_calibration_profile(char *profile) 
{
...
//remove by yewenliang, only support a device now
#if 0
...
#define TFA98XX_MAX_PROBE_CNT	3
#define TFA98XX_MAX_FIRMWARE_LOAD_CNT	20
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		...
		tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL_COUNT;
		if (try_cnt < TFA98XX_MAX_FIRMWARE_LOAD_CNT) {
			pr_info("%s: reload firmware try_cnt = %d\n", __func__, try_cnt);
	...
	if (tfa98xx->handle < 0) {
		...
		tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL_REG_DSP;
		return;
	}

	try_cnt = 0;
	while (try_cnt < TFA98XX_MAX_PROBE_CNT) {
		if (tfa_probe(tfa98xx->i2c->addr << 1, &handle) != Tfa98xx_Error_Ok) {
			dev_dbg(tfa98xx->dev, "Failed to probe TFA98xx @ 0x%.2x[%d]\n", tfa98xx->i2c->addr, try_cnt);
	...
	if (try_cnt >= TFA98XX_MAX_PROBE_CNT) {
		dev_err(tfa98xx->dev, "Failed to probe TFA98xx @ 0x%.2x\n", tfa98xx->i2c->addr);
		tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL_PROBE;
		return;
	}
...
static void tfa98xx_tapdet(struct tfa98xx *tfa98xx)
{
	...
	default:
		...
		if (tfa98xx_tapdet_report_check(tfa98xx) == 0) {
			input_report_rel(tfa98xx->input, EVENT_TYPE_TAPDET_VALUE, mem);
						...
						struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		tfa98xx->status = TFA98XX_PLAYBACK;
	else
		tfa98xx->status = TFA98XX_CAPTURE;
			     ...
			     struct snd_soc_dai *dai)
{
	...
	tfa98xx->rate = rate = params_rate(params);
	pr_debug("Requested rate: %d, sample size: %d\n", rate,
						snd_pcm_format_width(params_format(params)));
	val = tfa98xx_get_fssel(rate);
	if (val < 0) {
	...
	tfa98xx_profile = tfa98xx_mixer_profile;
	tfa98xx_vsteps[0] = tfa98xx_prof_vsteps[tfa98xx_profile];
	tfa98xx_vsteps[1] = tfa98xx_prof_vsteps[tfa98xx_profile];
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (!(tfa98xx->flags & TFA98XX_FLAG_DSP_START_ON_MUTE)) {
		pr_debug("TFA98XX_FLAG_DSP_START_NOT_MUTE\n");
		return 0;
	}
	...
	if (mute) {
		...
		if (tfa98xx->rate != 16000) {							//normal mode
			int err;
			mutex_lock(&tfa98xx->dsp_lock);
		...
		/* Start DSP */
		if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_PENDING && reschedule)
			queue_delayed_work(tfa98xx->tfa98xx_wq,
							&tfa98xx->init_work,
							0);
	}
...
extern enum Tfa98xx_Error tfa98xx_powerdown(Tfa98xx_handle_t handle, int powerdown);

static ssize_t tfa98xx_calibrate_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct tfa98xx *tfa98xx = dev_get_drvdata(dev);
	enum Tfa98xx_Error ret;
	int16_t is_started = 0;
	int profile = 0;

	if (strncmp(buf, "cal", strlen("cal"))) {
		pr_err("%s: buf(%s) is not equal cal, don't calibrate.\n", __func__, buf);
		return -EINVAL;
	}
	...
	if (ret) {
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_err("%s:tfa98xx_open failed(%d)\n", __func__, ret);
		return -EBUSY;
	}

	/* Do a full startup */
	ret = tfaRunStartup(tfa98xx->handle, profile);
	if (ret) {
		pr_err("%s:tfaRunStartup failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}
	...
	ret = tfa98xx_set_mtp(tfa98xx->handle,
			...
			TFA98XX_KEY2_PROTECTED_MTP0_MTPEX_MSK);
	if (ret) {
		pr_err("%s:tfa98xx_set_mtp failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}
	...
	if (ret) {
		pr_err("%s:tfaRunColdboot failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}

	/* if CF is in bypass then return here */
	if (TFA_GET_BF(tfa98xx->handle, CFE) == 0) {
		pr_err("%s: CF in bypass!\n", __func__);
		ret = -EINVAL;
		goto error_exit;
	}
	...
	if (ret) {
		pr_err("%s:tfaRunStartDSP failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}
	...
	if (ret) {
		pr_err("%s:tfaContWriteFiles failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}

	/* write all the files from the profile list (typically preset) */
	ret = tfaContWriteFilesProf(tfa98xx->handle, profile, 0); /* use volumestep 0 */
	if (ret) {
		pr_err("%s:tfaContWriteFilesProf failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}
	...
	tfa_set_swprof(tfa98xx->handle, (unsigned short)profile);
	tfa_set_swvstep(tfa98xx->handle, 0);

	/* Check if CF is not in bypass */
	if (TFA_GET_BF(tfa98xx->handle, CFE) == 0) {
		pr_err("%s: It is not possible to calibrate with CF in bypass!\n", __func__);
		ret = -EINVAL;
		goto error_exit;
	}
	...
	/* calibrate */
	ret = tfaRunSpeakerCalibration(tfa98xx->handle);
	if (ret) {
		pr_err("%s:tfaRunSpeakerCalibration failed(%d)\n", __func__, ret);
		ret = -EINVAL;
		goto error_exit;
	}

	pr_info("%s: Calibration complete, impedance is %d mOhms\n",
		__func__, handles_local[tfa98xx->handle].mohm[0]);

	ret = count;

error_exit:
	tfa98xx_powerdown(tfa98xx->handle, 1);
	tfa98xx_close(tfa98xx->handle);

	if (is_started) {
		int err;
		pr_info("%s: restart dsp, tfa98xx_profile is %d\n", __func__, tfa98xx_profile);
		err = tfa98xx_tfa_start(tfa98xx, tfa98xx_profile, tfa98xx_vsteps);
		if (err == Tfa98xx_Error_Ok)
			tfa98xx->dsp_init = TFA98XX_DSP_INIT_DONE;
	}
...
static ssize_t tfa98xx_impedance_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
//	enum Tfa98xx_Error error = Tfa98xx_Error_Ok;
	struct tfa98xx *tfa98xx = dev_get_drvdata(dev);

//	error = tfa_dsp_get_calibration_impedance(tfa98xx->handle);
//	if (error == Tfa98xx_Error_Ok) {
//		return snprintf(buf, PAGE_SIZE, "%d", handles_local[tfa98xx->handle].mohm[0]);
//	} else {
//		return -EIO;
//	}
	return snprintf(buf, PAGE_SIZE, "%d", handles_local[tfa98xx->handle].mohm[0]);
...
static DEVICE_ATTR(calibrate,	S_IWUSR | S_IRUGO, NULL, tfa98xx_calibrate_store);
static DEVICE_ATTR(impedance,	S_IWUSR | S_IRUGO, tfa98xx_impedance_show, NULL);

static struct attribute *tfa98xx_attributes[] = {
	&dev_attr_calibrate.attr,
	&dev_attr_impedance.attr,
	NULL
};

static struct attribute_group tfa98xx_attribute_group = {
	.attrs = tfa98xx_attributes
};
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "Failed to read Revision register: %d\n",
			ret);
		//MEIZU_M80 Force Register Codec
		//return -EIO;
		reg = 0x92;		//select tfa9891
	}
	switch (reg & 0xff) {
	case 0x88: /* tfa9888 */
...
//remove by yewenliang, only support a device now
	...
	//added by yewenliang
	//if (i2c->dev.of_node)
	//	dev_set_name(&i2c->dev, "%s", "tfa98xx");
	...
	if (tfa98xx->flags & TFA98XX_FLAG_TAPDET_AVAILABLE) {
		ret = tfa98xx_tapdet_register_device(tfa98xx);
		if (ret) {
	        pr_err("%s: register tapdet device Fail:%d\n", __func__, ret);
	        goto err_off;
		}
	}

	dev_set_drvdata(&i2c->dev, tfa98xx);
	ret = sysfs_create_group(&i2c->dev.kobj, &tfa98xx_attribute_group);
	if (ret < 0) {
		dev_info(&i2c->dev, "error creating sysfs attr files\n");
	} else {
		meizu_sysfslink_register_name(&i2c->dev, "tfa98xx");
	}

	tfa98xx->status = TFA98XX_STANDBY;
	tfa98xx->tapdet_active = false;

#ifdef TFA98XX_I2C_DEV_SUPPORT
    /* register MISC device */
	ret = tfa98xx_i2c_register_device(tfa98xx);
    if (ret) {
        pr_err("tfa98xx_i2c_init misc_register Fail:%d\n", ret);
        goto err_tapdet;
    }
#endif
...
err_tapdet:
if (tfa98xx->flags & TFA98XX_FLAG_TAPDET_AVAILABLE) {
	tfa98xx_tapdet_deregister_device(tfa98xx);
}
err_off:
...
#ifdef TFA98XX_I2C_DEV_SUPPORT
	tfa98xx_i2c_deregister_device(tfa98xx);
#endif
...
static int __init tfa98xx_i2c_init(void)
{
	...
	if (ret) {
        pr_err("fail to add tfa98xx device into i2c\n");
        return ret;
    }
