 ...
 */
enum Tfa98xx_DMEM tfa98xx_filter_mem(Tfa98xx_handle_t dev, int filter_index, unsigned short *address) 
{
	enum Tfa98xx_DMEM  dmem=-1;
	int idx;
	unsigned short bq_table[5][4] ={
	/* index: 10, 11, 12, 13 */
...
enum Tfa98xx_Error tfa98xx_set_volume_level(Tfa98xx_handle_t handle, unsigned short vol)
{
	...
	TFA_SET_BF_VALUE(handle, VOL, (uint16_t)vol, &reg_value);
	error = -TFA_WRITE_REG(handle, VOL, reg_value);
	return error;
}
...
enum Tfa98xx_Error tfaRunSpeakerBoost(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	trace_printk("%s %sstart\n",
	             ...
	             value ? "cold" : "warm");
#endif
	tap_profile = tfaContGetTapProfile(handle);

	if ((value == 1) && (profile != tap_profile)) {
...
enum Tfa98xx_Error tfaRunWaitCalibration(Tfa98xx_handle_t handle, int *calibrateDone)
{
	...
	*calibrateDone = 0;
	...
	/* Check which speaker calibration failed. Only for 88N1C */
	if((err != Tfa98xx_Error_Ok) && (handles_local[handle].rev == 0x1c88)) {
...
enum tfa_error tfa_start(int next_profile, int *vstep)
{
	...
	int cal_profile = -1, tap_profile = -1, active_profile = -1;
	...
	for( dev=0; dev < devcount; dev++) {
		...
		/* Search if there is a tap profile
		 * Only if the user did not give a specific profile and coldstart
		 */
		tap_profile = tfaContGetTapProfile(dev);
	...
	for( dev=0; dev < devcount; dev++) {
		...
		if (( next_profile != active_profile && active_profile != -1)
			|| next_profile == tap_profile) {
			err = tfaContWriteProfile(dev, next_profile, vstep[dev]);
			if (err!=Tfa98xx_Error_Ok)
				goto error_exit;
		} else if (vstep[dev] != tfaContGetCurrentVstep(dev) && vstep[dev] != -1) {
			err = tfaContWriteFilesVstep(dev, next_profile, vstep[dev]);
			if ( err != Tfa98xx_Error_Ok)
				goto error_exit;
		}

		if (TFA_GET_BF(dev, PWDN) != 0) {
