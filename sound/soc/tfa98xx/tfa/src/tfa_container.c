...
enum Tfa98xx_Error tfaRunWriteFilter(Tfa98xx_handle_t dev, nxpTfaContBiquad_t *bq) {
	...
	dmem = tfa98xx_filter_mem(dev, bq->aa.index, &address);
	if(dmem<0)
		return Tfa98xx_Error_Bad_Parameter;
	...
	if ( tfa98xx_cnt_verbose ) {
		if (bq->in.index==13) {
			pr_debug("filter[%d]=%d,%.0f,%.2f\n", bq->in.index, bq->in.type,
						bq->in.cutOffFreq, bq->in.leakage);
		} else if ( (bq->aa.index==10) | (bq->aa.index==11) | (bq->aa.index==12) ) {
			pr_debug("filter[%d]=%d,%.0f,%.1f,%.1f\n", bq->aa.index, bq->aa.type,
					bq->aa.cutOffFreq, bq->aa.rippleDb, bq->aa.rolloff);
		} else {
			pr_debug("unsupported filter index:%d\n", bq->aa.index);
		}
	}
#endif
...
enum Tfa98xx_Error tfaContWriteItem(int dev_idx, nxpTfaDescPtr_t * dsc) {
	...
	case dscPatch:
		pr_debug("Unable to write patch! \n");
		//file = (nxpTfaFileDsc_t *)(dsc->offset+(uint8_t *)g_cont);
		//if ( tfa98xx_cnt_verbose )
			//TODO do we want dscPatch verbose here?
...
/*
 * Get profile index of the tap profile. 
 * Returns: (profile index) if found, (-2) if no
 * calibration profile is found or (-1) on error
 */
int tfaContGetTapProfile(int dev_idx) {
	int prof, nprof, tap_idx = -2;
	...
	/* search for the calibration profile in the list of profiles */
	for (prof = 0; prof < nprof; prof++) {
		if(strstr(tfaContProfileName(dev_idx, prof), "tap") != NULL) {	//.tap
			tap_idx = prof;
