						...
						void *buffer, size_t len)
{
	...
	//start = 524288 / MMC_BLOCK_SIZE;
	//nr_sect = 7340032 / MMC_BLOCK_SIZE;
...
static int mmc_blk_probe(struct mmc_card *card)
{
	...
	if (card->type != MMC_TYPE_SD_COMBO) {
       ...
       if (!strcmp(md->disk->disk_name, "mmcblk0")) {
               ...
               disk_part_iter_init(&piter, emmc_disk, DISK_PITER_INCL_EMPTY);
               while ((part = disk_part_iter_next(&piter))) {
                       if (part->info && (!strcmp(part->info->volname, "devinfo"))) {
                               part->policy = 1;
                               continue;
                       }
                       part->policy = 0;
               ...
               disk_part_iter_init(&piter, emmc_disk, DISK_PITER_INCL_EMPTY);
               while ((part = disk_part_iter_next(&piter))) {
                       if (part->info && (!strcmp(part->info->volname, "cache") ||
                               !strcmp(part->info->volname, "system") ||
                               !strcmp(part->info->volname, "custom") ||
                               !strcmp(part->info->volname, "proinfo") ||
                               !strcmp(part->info->volname, "nvram") ||
                               !strcmp(part->info->volname, "secro") ||
                               !strcmp(part->info->volname, "expdb") ||
                               !strcmp(part->info->volname, "protect1") ||
                               !strcmp(part->info->volname, "protect2") ||
                               !strcmp(part->info->volname, "recovery") ||
                               !strcmp(part->info->volname, "rstinfo") ||
                               !strcmp(part->info->volname, "seccfg") ||
                               !strcmp(part->info->volname, "tee1") ||
                               !strcmp(part->info->volname, "tee2") ||
                               !strcmp(part->info->volname, "para") ||
                               !strcmp(part->info->volname, "oemkeystore") ||
                               !strcmp(part->info->volname, "keystore") ||
                               !strcmp(part->info->volname, "nvdata") ||
                               !strcmp(part->info->volname, "frp") ||
                               !strcmp(part->info->volname, "metadata") ||
