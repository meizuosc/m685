...
long lcd_vddi18_setting(unsigned int value)
{
	long ret = 0;

#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_DVDD1V8_ON);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_DVDD1V8_OFF);
#endif
...
int ddp_dsi_set_lcm_utils(DISP_MODULE_ENUM module, LCM_DRIVER *lcm_drv)
{
	...
	utils->set_gpio_lcd_vddi18 = lcd_vddi18_setting;
#endif
#endif
