...
#ifdef MEIZU_M80
unsigned int primary_display_get_panel_id(void)
{
	return 0;
}

unsigned int primary_display_get_tp_id(void)
{
	return 0;
}
void primary_display_trigger_frame(void)
{
    DISPFUNC();
    ...
    if (primary_get_state() == DISP_SLEPT) {
        DISPCHECK("primary_display_trigger_frame, but primary display path is slept??\n");
        goto done;
    }
...
int primary_display_sethbm(unsigned int onoff)
{
	...
	{
		if (lcm_drv->set_hbm) {
			primary_display_trigger_frame();
				lcm_drv->set_hbm(onoff);
				primary_display_trigger_frame();
				}
		//primary_display_trigger(0, NULL, 0);
...
int primary_display_setlut(unsigned int val)
{
	DISPFUNC();
	int ret = 0;
	LCM_DRIVER *lcm_drv = pgc->plcm->drv;
