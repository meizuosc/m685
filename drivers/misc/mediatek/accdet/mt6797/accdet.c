...
#ifdef MEIZU_M80
#define MEIZU_M80_KEY_CALIBRATION

#define FIX_WORK_DELAY_TIME	1200  /* delay 1.2s and then run fix callback */
static struct delayed_work weird_headphone_fix_work;
static struct workqueue_struct *weird_headphone_fix_workqueue = NULL;

#ifdef CONFIG_SND_SOC_CS43L36
static bool is_weird_headphone = false;
static bool micbias_enable = false;
static bool impedance_detected = false;
static DEFINE_MUTEX(accdet_micbias_mutex);
#endif
...
/******************Add for calibration ******************/
#ifdef MEIZU_M80_KEY_CALIBRATION
#define STATE_LEN	10
...
#ifdef MEIZU_M80
#ifdef CONFIG_SND_SOC_CS43L36
extern int cs43l36_load_detection(struct device *dev);
void accdet_get_impedance(struct device *dev)
{
	if (impedance_detected) {
		pr_debug("%s: headphone shouldn't detect impedance.\n", __func__);
		return;
	}

	mutex_lock(&accdet_micbias_mutex);
	pr_info("%s: is_weird_headphone(%d), micbias_enable(%d)\n", __func__, is_weird_headphone, micbias_enable);

	if (is_weird_headphone || !micbias_enable) {
		cs43l36_load_detection(dev);
	} else {
		/*setting pwm idle;*/
	...
	mutex_unlock(&accdet_micbias_mutex);

	impedance_detected = true;
}
EXPORT_SYMBOL(accdet_get_impedance);
#endif
#endif
...
#ifdef CONFIG_SND_SOC_CS43L36
	is_weird_headphone = true;
	impedance_detected = false;
#endif
...
static void accdet_workqueue_func(void)
{
	...
	ACCDET_DEBUG("[Accdet]cancel weird headphone fix work\n");
	cancel_delayed_work_sync(&weird_headphone_fix_work);
#endif
...
static void accdet_work_callback(struct work_struct *work)
{
    ...
    if (calibration_state == CALIBRATION_PENDING) {
        pr_warn("%s(), accdet calibration is running, return ...\n", __func__);
        return;
    }
#endif
...
#ifdef CONFIG_SND_SOC_CS43L36
	mutex_lock(&accdet_micbias_mutex);
	if (is_weird_headphone == true) {
		pr_info("%s: pre state is weried headphone\n", __func__);
		is_weird_headphone = false;
		impedance_detected = false;
	}
	mutex_unlock(&accdet_micbias_mutex);
...
RETRY:
	for (i = 0; i < NUM; i++) {
		...
		if (voltage < accdet_dts_data.three_key.down_key && tmp_value <= voltage)
			tmp_value = voltage;
	}
	if (tmp_value < accdet_dts_data.three_key.mid_key || tmp_value >= accdet_dts_data.three_key.down_key) {
		if (retry++ <= RETRIES) { msleep(1); goto RETRY;}
		calibration_state = CALIBRATION_ERROR;
	} else {
		pr_info("%s: calibration complete, tmp_value is %d\n", __func__, tmp_value);
		accdet_dts_data.three_key.up_key = tmp_value + 40; /* Add 40mv as threshold */
		calibration_state = CALIBRATION_COMPLETE;
	}

	/*accdet_auxadc_switch(0);*/
#if 1//def CONFIG_ACCDET_EINT_IRQ
	pmic_pwrap_write(ACCDET_IRQ_STS, pmic_pwrap_read(ACCDET_IRQ_STS) & (~IRQ_EINT_CLR_BIT));
#endif
...
static ssize_t accdet_calibrate_cmd_show(struct device *dev,
...
static ssize_t accdet_calibrate_cmd_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	/* receive control command */
	pr_info("%s(), recive command: %s\n",__func__, buf);
...
static ssize_t accdet_calibrate_data_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int count = 0;

	sprintf(buf + count, "%d", accdet_dts_data.three_key.up_key);

	return	strlen(buf);
}

static ssize_t accdet_calibrate_data_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int err = 0;
	int up_key;
	...
	if (err == 1) {
		accdet_dts_data.three_key.up_key = up_key;
	} else {
		pr_err("%s(), parse buffer failed\n", __func__);
	}
	return	count;
}

static ssize_t accdet_calibrate_state_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int count;
...
#ifdef MEIZU_M80_KEY_CALIBRATION
	if ((ret = accdet_create_calibration_attr(&dev->dev))==0)
		meizu_sysfslink_register_name(&dev->dev, "accdet_calibration");
