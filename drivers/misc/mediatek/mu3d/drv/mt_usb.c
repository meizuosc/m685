...
#ifndef CONFIG_MZ_SMART_BATTERY
...
void connection_work(struct work_struct *data)
{
	...
	if (!usb_phy_check_in_uart_mode()) {
		...
		if ((get_boot_mode()==META_BOOT)&&!is_usb_rdy()) {
			 ...
			 const char *buf, size_t count)
{
	...
	if (!dev) {
		...
		if (cable_mode != cmode) {
			...
			set_usb_current_unlimited(cmode == CABLE_MODE_HOST_ONLY ? true : false);
			if (cmode == CABLE_MODE_CHRG_ONLY) {	/* IPO shutdown, disable USB */
				if (musb) {
					musb->usb_mode = CABLE_MODE_CHRG_ONLY;
					mt_usb_disconnect();
...
void musb_uart_usb_switch(unsigned int portmode)
{
	switch(portmode){
	case PORT_MODE_UART:
		pr_info("USB Port mode -> UART\n");
		if(port_mode == PORT_MODE_USB)
			usb_phy_switch_to_uart();
		port_mode = PORT_MODE_UART;
		break;
	case PORT_MODE_USB:
		pr_info("USB Port mode -> USB\n");
	...
	default:
		/*reverse usb mode*/
		portmode = port_mode;
		if (portmode == PORT_MODE_USB) {	/* Changing to UART Mode */
			pr_debug("USB Port mode -> UART\n");
			usb_phy_switch_to_uart();
			port_mode = PORT_MODE_UART;
		} else if (portmode == PORT_MODE_UART) {	/* Changing to USB Mode */
