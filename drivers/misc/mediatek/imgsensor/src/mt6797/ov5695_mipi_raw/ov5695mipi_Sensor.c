...
#define PFX "[ov5695_camera_sunny]"
#define LOG_1 LOG_INF("OV5695_SUNNY,MIPI 2LANE\n")
...
ov5695_otp_struct *ov5695_sunny_otp = NULL;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (NULL == ov5695_sunny_otp) {
					ov5695_sunny_otp = (ov5695_otp_struct *)kzalloc(sizeof(ov5695_otp_struct), GFP_KERNEL);
					if(NULL == ov5695_sunny_otp) {
						LOG_INF("ov5695 otp allocate mem failed\n");
						return -ENOMEM;
					}
				}
				memset(ov5695_sunny_otp, 0, sizeof(ov5695_otp_struct));
				ov5695_read_otp(ov5695_sunny_otp);
				module_id = ov5695_sunny_otp->module_id;
				if(SUNNY_MODULE_ID == module_id){
					*sensor_id = OV5695_SENSOR_ID;
					sub_module_id = OV5695_SENSOR_ID;
 					LOG_INF("get ov5695 sunny module success !! i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
#if OV5695_OTP_DEBUG
					LOG_INF("read otp data flag = 0x%x.\n", ov5695_sunny_otp->flag);
					LOG_INF("read otp data module id = 0x%x.\n", ov5695_sunny_otp->module_id);
					LOG_INF("read otp data lens id = 0x%x.\n", ov5695_sunny_otp->lens_id);
					LOG_INF("read otp data awb rg_msb = 0x%x.\n", ov5695_sunny_otp->awb_rg_msb);
					LOG_INF("read otp data awb bg_msb = 0x%x.\n", ov5695_sunny_otp->awb_bg_msb);
					LOG_INF("read otp data awb lsb = 0x%x.\n", ov5695_sunny_otp->awb_lsb);
#endif
					return ERROR_NONE;
				} else {
		...
		LOG_INF("Get ov5695 sunny module fail,i2c_write_id 0x%x,sensor id: 0x%x,module id:0x%x\n", imgsensor.i2c_write_id, *sensor_id, module_id);
		i++;
		retry = 2;
	}
	if (*sensor_id != imgsensor_info.sensor_id) {
