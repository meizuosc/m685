...
#define PK_INF(fmt, args...)     pr_info("[%s] " fmt, __FUNCTION__, ##args)
...
static ssize_t main_camera_type_show(struct device *dev,
		struct device_attribute *attr,char *buf)
{

	char *p = buf;
	
	if (main_module_id == IMX386_SENSOR_ID)
		p += sprintf(p, "imx386-primax\n");
	else if (main_module_id == IMX386SUNNY_SENSOR_ID)
		p += sprintf(p, "imx386-sunny\n");
	else
		p += sprintf(p, "null\n");
...
static ssize_t main_camera_type_store(struct device *dev,
...
static ssize_t sub_camera_type_show(struct device *dev,
		struct device_attribute *attr,char *buf)
{

	char *p = buf;
	
	if (sub_module_id == OV5695_SENSOR_ID)
		p += sprintf(p, "ov5695-sunny\n");
	else if (sub_module_id == OV5695TECH_SENSOR_ID)
		p += sprintf(p, "ov5695-qtech\n");
	else
		p += sprintf(p, "null\n");
...
static ssize_t sub_camera_type_store(struct device *dev,
		struct device_attribute *attr, const char *buf,size_t count)
{
...
static struct device_attribute dev_attr_main_camera_type = {

	.attr = {.name = "main_camera_type", .mode = 0644},
	.show = main_camera_type_show,
	.store= main_camera_type_store,
};

static struct device_attribute dev_attr_sub_camera_type = {

	.attr = {.name = "sub_camera_type", .mode = 0644},
	.show = sub_camera_type_show,
	.store= sub_camera_type_store,
};
...
inline static int RegisterCAMERA_HWCharDrv(void)
{
    ...
    device_create_file(sensor_device,&dev_attr_main_camera_type);
    device_create_file(sensor_device,&dev_attr_sub_camera_type);
    meizu_sysfslink_register_name(sensor_device, "cam_sensor");
    return 0;
}
...
static int CAMERA_HW_i2c_probe3(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /* set I2C clock rate */
    #ifdef CONFIG_MTK_I2C_EXTENSION
    g_pstI2Cclient3->timing = 400;/* 400k */
