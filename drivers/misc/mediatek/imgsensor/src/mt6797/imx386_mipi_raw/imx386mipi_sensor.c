...
{
	memcpy((void *)data,(void *)(primax_otp_buf + PDAF_CAL_DATA_OFFSET), PDAF_CAL_DATA_SIZE);
}
...
static void load_imx386_spc_data(void)
{
	...
	{
		write_cmos_sensor(0x7D4C+i, primax_otp_buf[SPC1_START_ADDR + i]);
	}
	for(i = 0; i < SPC_DATA_SIZE; i++)
	{
		write_cmos_sensor(0x7D80+i, primax_otp_buf[SPC2_START_ADDR + i]);
	}
}
...
otp_read:
	...
	imx386_read_otp_burst(OTP_START_ADDR, primax_otp_buf);
