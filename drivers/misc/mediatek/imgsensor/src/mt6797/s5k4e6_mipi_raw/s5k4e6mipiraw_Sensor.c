...
#define PFX "s5k4e6_sunny_sensor"
#define LOG_1 LOG_INF("s5k4e6_sunny,MIPI 2LANE\n")
...
s5k4e6_otp_struct *sunny_otp_data;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if (read_sensor_id == 0x4E60) {
				/*get module ID,if sunny module,then read otp*/
				module_id = s5k4e6_get_module_id();
				if(0x01 == module_id){ 
				sunny_otp_data = (s5k4e6_otp_struct *)kzalloc(sizeof(s5k4e6_otp_struct), GFP_KERNEL);
				...
				*sensor_id = imgsensor_info.sensor_id;
				...
				s5k4e6_read_otp_all(sunny_otp_data);
				printk("get s5k4e6 sunny module success !! i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
				}
			}
			retry--;
		} while(retry > 0);
		printk("Get s5k4e6 sunny module fail,i2c_write_id 0x%x,sensor id: 0x%x,module id:0x%x\n", imgsensor.i2c_write_id,read_sensor_id,module_id);
		i++;
		retry = 2;
	}
