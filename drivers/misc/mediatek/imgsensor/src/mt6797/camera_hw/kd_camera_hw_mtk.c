...
#else //meizu m80 camera power define
	  {SENSOR_DRVNAME_IMX230_MIPI_RAW,
	  {
	   {SensorMCLK, Vol_High, 0},
	   {AVDD, Vol_High, 1},
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case PDN:
		...
		break;
	case AVDD:
#ifdef MEIZU_M80 //M80 imx230 use GPIO30 to control AVDD
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (mtkcam_gpio_set(pinSetIdx, MAIN2_AVDD, PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Voltage)) {
						PK_INFO("[CAMERA CUST_AVDD] set MAIN AVDD gpio failed!!\n");
				}
			}
		}
		else if(pinSetIdx == 1){
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
					PK_ERR("[CAMERA SENSOR] Fail to enable SUB_AVDD power\n");
					return FALSE;
				}
			} else {
			...
			if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
					PK_ERR("[CAMERA SENSOR] Fail to enable MAIN_AVDD power\n");
					return FALSE;
				}
			} else {
		...
		if (pinSetIdx == 2) {
			...
			if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (TRUE != _hwPowerOn(DVDD, pwInfo.Voltage)) {
					PK_ERR("[CAMERA SENSOR] Fail to enable MAIN_DVDD power\n");
					return FALSE;
				}
			} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
				PK_ERR("[CAMERA SENSOR] Fail to enable DOVDD power\n");
				return FALSE;
			}
		} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
				PK_ERR("[CAMERA SENSOR] Fail to enable AFVDD power\n");
				return FALSE;
			}
		} else {
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disable SUB_AVDD power\n");
					return FALSE;
				}
			} else {
			...
			if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disable MAIN_AVDD power\n");
					return FALSE;
				}
			} else {
			...
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				...
				if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
					if (TRUE != _hwPowerDown(SUB_DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disable SUB_DVDD power\n");
						return FALSE;
					}
				} else {
				...
				if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
					if (TRUE != _hwPowerDown(DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disable MAIN_DVDD power\n");
						return FALSE;
					}
				} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
				PK_ERR("[CAMERA SENSOR] Fail to disable DOVDD power\n");
				return FALSE;
			}
		} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
				PK_ERR("[CAMERA SENSOR] Fail to disable AF_DVDD power\n");
				return FALSE;
			}
		} else {
...
/*This function is to Enable Laser to achive I2C communication*/
int Laser_Enable(bool on){

	if(on){
		/*Pull up Laser Enable GPIO*/
		pinctrl_select_state(camctrl, cam0_laser_h);
			
		/*Power up IO for I2C transfer*/
		...
		/*Pull down Laser Enable GPIO*/
		pinctrl_select_state(camctrl, cam0_laser_l);
...
#ifdef MEIZU_M80  //meizu m80 define power
...
/* power ON */
	if (On) {
		...
		//	}
			...
			if (TRUE != _hwPowerOn(DVDD,Vol_1000)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable AFVDD power\n");
				goto _kdCISModulePowerOn_exit_;
			}
			...
			if (TRUE != _hwPowerOn(SUB_DVDD,Vol_1200)) {
				PK_DBG("[CAMERA SENSOR] Fail to enable SUB_DVDD power\n");
				goto _kdCISModulePowerOn_exit_;
			}
