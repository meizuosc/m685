...
#include "ov5695techmipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "[ov5695_camera_qtech]"
#define LOG_1 LOG_INF("OV5695_QTECH,MIPI 2LANE\n")
...
#define OV5695_OTP_DEBUG	1
#define OV5695MIPIYUV_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, 0x78)
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5695TECH_SENSOR_ID & 0x5FFF,		//record sensor id defined in Kd_imgsensor.h
...
{{ 2624, 1980,	  0,	0, 2624, 1972, 2624, 1972, 0000, 0000, 2624,  1972,	 16,	8, 2592, 1944}, // Preview 
 { 2624, 1980,	  0,	0, 2624, 1972, 2624, 1972, 0000, 0000, 2624,  1972,	 16,	8, 2592, 1944}, // capture 
 { 2624, 1980,	  0,	0, 2624, 1972, 2624, 1972, 0000, 0000, 2624,  1972,	 16,	8, 2592, 1944}, // video 
...
ov5695_otp_struct *ov5695_qtech_otp = NULL;
#define QTECH_MODULE_ID	0x06

static int ov5695_read_otp(ov5695_otp_struct *pOtp)
{

	u16 otp_flag, addr, temp1, i;
	...
	if(addr != 0) {
		...
		pOtp->awb_rg_msb = read_cmos_sensor( addr + 2);
		pOtp->awb_bg_msb = read_cmos_sensor( addr + 3);
		pOtp->awb_lsb = read_cmos_sensor( addr + 4);
	}
	else {
...
static void write_shutter(kal_uint16 shutter)
{
	...
	if(shutter > (imgsensor_info.max_frame_length - imgsensor_info.margin)) {
		line_length = shutter * 740 / (imgsensor_info.max_frame_length - imgsensor_info.margin);
	...
	LOG_INF("Exit! shutter =%d, framelength =%d, long_exp_line_length:%d.\n", shutter,imgsensor.frame_length, line_length);
}	/*	write_shutter  */
...
//;mipi_datarate per lane: 840Mbps
		...
		write_cmos_sensor(0x0302, 0x7D);	//MIPI clk= 1000M
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				if (NULL == ov5695_qtech_otp) {
					ov5695_qtech_otp = (ov5695_otp_struct *)kzalloc(sizeof(ov5695_otp_struct), GFP_KERNEL);
					if(NULL == ov5695_qtech_otp) {
						LOG_INF("ov5695 otp allocate mem failed\n");
						return -ENOMEM;
					}
				}
				memset(ov5695_qtech_otp, 0, sizeof(ov5695_otp_struct));
				ov5695_read_otp(ov5695_qtech_otp);
				module_id = ov5695_qtech_otp->module_id;
				if(QTECH_MODULE_ID == module_id){
					*sensor_id = OV5695TECH_SENSOR_ID;
					sub_module_id = OV5695TECH_SENSOR_ID;
 					LOG_INF("get ov5695 qtech module success !! i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
#if OV5695_OTP_DEBUG
					LOG_INF("read otp data flag = 0x%x.\n", ov5695_qtech_otp->flag);
					LOG_INF("read otp data module id = 0x%x.\n", ov5695_qtech_otp->module_id);
					LOG_INF("read otp data lens id = 0x%x.\n", ov5695_qtech_otp->lens_id);
					LOG_INF("read otp data awb rg_msb = 0x%x.\n", ov5695_qtech_otp->awb_rg_msb);
					LOG_INF("read otp data awb bg_msb = 0x%x.\n", ov5695_qtech_otp->awb_bg_msb);
					LOG_INF("read otp data awb lsb = 0x%x.\n", ov5695_qtech_otp->awb_lsb);
#endif
					return ERROR_NONE;
				} else {
		...
		LOG_INF("Get ov5695 qtech module fail,i2c_write_id 0x%x,sensor id: 0x%x,module id:0x%x\n", imgsensor.i2c_write_id, *sensor_id, module_id);
		i++;
		retry = 2;
	}
	if (*sensor_id != imgsensor_info.sensor_id) {
...
UINT32 OV5695TECH_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5695TECH_MIPI_RAW_SensorInit	*/
