...
#define BAT_TASK_CHECKTYPE_PERIOD                     (5)/* 5s */
#define BAT_TASK_POWEROFF_PERIOD                     (10)/* 10s */
#define BAT_TASK_DEFAULT_PERIOD                     (30)/* 30sec */
#define BAT_TASK_SUSPEND_PERIOD                     (900)/* 15min */
...
#if defined(CONFIG_MTK_BQ2589X_SUPPORT)
typedef enum {
	BQ2589X_NOT_CHARGE,
	BQ2589X_PRE_CHARGE,
	BQ2589X_FAST_CHARGE,
...
typedef struct {
	...
	int chrg_stat;
	kal_bool qc_connect; 
	kal_bool vbus_cable_out_occur;
	kal_bool qc_check_flag;
	kal_bool ico_check;
	int g_platform_boot_mode;
	int g_qcct_flag;
	int g_bcct_flag;
	int g_bcct_input_flag;
	int batt_status; 
	int pre_batt_status; 
	int voltage_up;
	int adapter_num;
...
extern struct wake_lock chgin_wakelock;
#if defined(CONFIG_MTK_BQ2589X_SUPPORT)
extern void mtk_ta_reset_vchr(void);
extern int ta_v_chr_org;
extern bool fast_charger_detect;
extern bool battery_soc_less_30;
...
extern void mt_battery_disable_charging_algorithm(void);
extern int fb_event_notify(struct notifier_block *this, unsigned long code,void *data);
...
extern int get_mtk_bts_temp(void);
extern int get_mtk_charger_temp(void);
extern int get_mtk_usb_board_temp(void);
extern void set_bat_thread_priority(unsigned short pri_val);
