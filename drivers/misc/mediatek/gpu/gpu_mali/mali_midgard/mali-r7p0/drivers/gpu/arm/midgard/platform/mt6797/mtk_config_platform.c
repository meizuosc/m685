...
static struct regulator *g_gpu_regulator;
...
static int _dump_mfg_n_ldo_registers(unsigned int mtklog, int in_irq)
{
	if (!in_irq)
	{
		...
		enable = regulator_is_enabled(g_gpu_regulator);
		voltage = regulator_get_voltage(g_gpu_regulator);

		PRINT_LOGS(mtklog,
				"[dump][gpu buck enable = %d][voltage = %d uV]", enable, voltage);
	}
...
int kbase_platform_early_init(void)
{
	...
	g_gpu_regulator = mt_gpufreq_get_vgpu_buck(&vgpu_buck_id);
	if(IS_ERR(g_gpu_regulator)){
		pr_err("vgpu_buck regulator no ready\n");
		return -EPROBE_DEFER;
	}
	return 0;
}
