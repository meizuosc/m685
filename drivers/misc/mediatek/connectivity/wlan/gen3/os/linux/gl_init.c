...
static void __wlanSetMulticastList(void)
{
	struct net_device * wlan0 = NULL ;

	wlan0 = dev_get_by_name(&init_net,"wlan0");
	if(!wlan0){
		printk(KERN_ERR"%s:get wlan0 error\n",__FUNCTION__);
		return ;
	}
	wlanSetMulticastList(wlan0);
	dev_put(wlan0);
}
...
static void wlanSetMulticastListWorkQueue(struct work_struct *work)
{
	...
	if (kalIoctl(prGlueInfo,
      ...
      /*Don't set Multicast address to FW in suspend Mode */
	if (!prGlueInfo->fgIsInSuspendMode&&(u4PacketFilter & PARAM_PACKET_FILTER_MULTICAST)) {
...
#ifdef MEIZU_M80
		/* 2016-1-28 Add by MEIZU BSP@YinshunQing for fixed null pointer reboot issue */
		if (!prMCAddrList) {
			DBGLOG(INIT, ERROR, "failed to alloc memory for prMCAddrList\n");
			kalHaltUnlock();
			return;
		}
#endif
...
static INT_32 wlanProbe(PVOID pvData)
{
	...
	do {
        ...
        if(!prGlueInfo->prAdapter){
			DBGLOG(INIT, ERROR, "wlanProbe: No memory for prAdapter and its private\n");
			i4Status = -ENOMEM;
			eFailReason = NET_CREATE_FAIL;
			break;
        }
