		      ...
		      struct cfg80211_scan_request *request)
{
	...
	/* 2016-01-22 Add by MEIZU BSP@YinShunQing for print WIFI received scan cmd for userspace */
	pr_info("%s : Received userspace scan command\n", __func__);
...
int mtk_cfg80211_connect(struct wiphy *wiphy, struct net_device *ndev, struct cfg80211_connect_params *sme)
{
	...
	/* 2016-01-22 Add by MEIZU BSP@YinShunQing for print WIFI received connect cmd from userspace */
	pr_info("%s : Received userspace connect command\n", __func__);
...
/* fgIsTDLSlinkEnable = 0; */
	...
	/* 2016-2-3 Removed by MEIZU BSP@YinshunQing for fixed klocwork critical issue */
#ifndef MEIZU_M80
	prAdapter = prGlueInfo->prAdapter;
#endif
	prGlueInfo = (P_GLUE_INFO_T) wiphy_priv(wiphy);
