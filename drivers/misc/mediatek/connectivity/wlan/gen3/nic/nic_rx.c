	...
	case EVENT_ID_GSCAN_FULL_RESULT:
		{
			...
			if (!prEventGscnFullResult) {
				DBGLOG(SCN, WARN, "failed to alloc memory for prEventGscnFullResult!\n");
				break;
			}
#endif
			...
			if (!prParamGscnFullResult) {
				DBGLOG(SCN, WARN, "failed to alloc memory for prParamGscnFullResult!\n");
				break;
			}
#endif
...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	case EVENT_ID_BSS_BEACON_TIMEOUT:
		...
		if (prAdapter->fgDisBcnLostDetection == FALSE) {
			...
			/* 2016-1-27 Add by MEIZU BSP@YinShunQing for print beacon timeout reason */
#ifdef MEIZU_M80
			pr_info("%s: Beacon Timeout Reason: %d\n", __func__, prEventBssBeaconTimeout->ucReasonCode);
#else
