...
********************************************************************************
*/
//FLYME:lizilai@wlan.modify add for get hardware ver @{
#define HARDWAREVER_FILE_BUF_SIZE 128

#define MAX_HARDWAREVER_WITHOUT_CLASS115 4
...
static int getHardwareVer(const char* path)
{
    PUINT_8 pucConfigBuf = NULL;
    UINT_32 u4ConfigReadLen = 0;
    UINT_8 ucHardwareVer = 0;

    pucConfigBuf = (PUINT_8) kalMemAlloc(HARDWAREVER_FILE_BUF_SIZE, VIR_MEM_TYPE);
    if (pucConfigBuf == NULL) {
        printk("get hardware version malloc failed\n");
        return ucHardwareVer;
    }

    kalMemZero(pucConfigBuf, HARDWAREVER_FILE_BUF_SIZE);

    if (kalReadToFile(path, pucConfigBuf,
           	HARDWAREVER_FILE_BUF_SIZE, &u4ConfigReadLen) != 0) {
        printk("read version failed\n");
        return ucHardwareVer;
    }

    printk("pucConfigBuf is [%s] \n", pucConfigBuf);
    //version code  is a string looks like "0x4", "0xa",maybe "0x1a"
	if (pucConfigBuf[0] == '0'
            && (pucConfigBuf[1] == 'x' || pucConfigBuf[1] == 'X')) {
	    if (u4ConfigReadLen == 4) {
		    ucHardwareVer = hexCharToNum(pucConfigBuf[2]);
	    } else if (u4ConfigReadLen == 5) {
   		    ucHardwareVer = (hexCharToNum(pucConfigBuf[2]) << 4) + hexCharToNum(pucConfigBuf[3]);
	    }
    }

    kalMemFree(pucConfigBuf, VIR_MEM_TYPE, HARDWAREVER_FILE_BUF_SIZE);

    // only bit[0,1,2] is hardware version code
    ucHardwareVer &= 0x07;

    printk("hardwareVer is %d \n", ucHardwareVer);
    return ucHardwareVer;
...
*/
/*----------------------------------------------------------------------------*/
DOMAIN_INFO_ENTRY g_DomainInfoEntryWithoutClass115;
...
//FLYME:lizilai@wlan.modifyi  for prohibit WIFI Channel class 115 @{
#if 1
	return prDomainInfo;
#else
	hardwareVer = getHardwareVer("/proc/lk_info/hw_version");

	if (hardwareVer <= MAX_HARDWAREVER_WITHOUT_CLASS115) {
		kalMemCopy(&g_DomainInfoEntryWithoutClass115, prDomainInfo, sizeof(g_DomainInfoEntryWithoutClass115));
		UINT_8 ucSubBandSize = sizeof(g_DomainInfoEntryWithoutClass115.rSubBand)
			/ sizeof(g_DomainInfoEntryWithoutClass115.rSubBand[0]);
		for (i = 0; i < ucSubBandSize; i++) {
			if (g_DomainInfoEntryWithoutClass115.rSubBand[i].ucRegClass == 115) {
				g_DomainInfoEntryWithoutClass115.rSubBand[i].ucBand = BAND_NULL;
				g_DomainInfoEntryWithoutClass115.rSubBand[i].ucChannelSpan = 0;
				g_DomainInfoEntryWithoutClass115.rSubBand[i].ucFirstChannelNum = 0;
				g_DomainInfoEntryWithoutClass115.rSubBand[i].ucNumChannels = 0;
				g_DomainInfoEntryWithoutClass115.rSubBand[i].fgDfs = FALSE;
				break;
			}
		}
		prAdapter->prDomainInfo = &g_DomainInfoEntryWithoutClass115;
	}
