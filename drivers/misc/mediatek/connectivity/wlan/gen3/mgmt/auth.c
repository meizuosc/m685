			   ...
			   IN P_SW_RFB_T prSwRfb, IN UINT_16 u2TransactionSeqNum, OUT PUINT_16 pu2StatusCode)
{
	...
	if (u2RxAuthAlgNum != (UINT_16) prStaRec->ucAuthAlgNum) {
		DBGLOG(SAA, INFO, "Discard Auth frame with auth type = %d, current = %d\n",
	...
	if (u2RxTransactionSeqNum != u2TransactionSeqNum) {
		DBGLOG(SAA, INFO, "Discard Auth frame with Transaction Seq No = %d\n",
		    ...
		    IN P_SW_RFB_T prClassErrSwRfb, IN UINT_16 u2ReasonCode, IN PFN_TX_DONE_HANDLER pfTxDoneHandler)
{
	...
	/* 2016-1-27 Add by MEIZU BSP@YinShunQing for print send deauth frame log */
#ifdef MEIZU_M80
	if (!prStaRec) {
		pr_info("%s: Send Deauth Frame.\n", __func__);
	} else {
		pr_info("%s: Send Deauth Frame to MAC:["MACSTR"].\n", __func__, MAC2STR(prStaRec->aucMacAddr));
	}
#endif
