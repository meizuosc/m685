...
VOID scnInit(IN P_ADAPTER_T prAdapter)
{
	...
	if (prScanInfo->prPscnParam) {
		...
		DBGLOG(SCN, ERROR, "failed to alloc memory for PSCN param\n");
		return;
	}
#else
	kalMemZero(prScanInfo->prPscnParam, sizeof(PSCN_PARAM_T));
...
WLAN_STATUS scanAddScanResult(IN P_ADAPTER_T prAdapter, IN P_BSS_DESC_T prBssDesc, IN P_SW_RFB_T prSwRfb)
{
	...
	/* 2016-01-22 Add by MEIZU BSP@YinShunQing for print WIFI scan result start */
#ifndef MEIZU_M80
	DBGLOG(SCN, TRACE, "ind %s %d %d\n", prBssDesc->aucSSID, prBssDesc->ucChannelNum, prBssDesc->ucRCPI);
#else
	DBGLOG(SCN, INFO, "Scanresult: SSID[%s] MAC["MACSTR"] ChannelNum = %d RCPI = %d\n", prBssDesc->aucSSID,
			MAC2STR(prBssDesc->aucBSSID),
			prBssDesc->ucChannelNum,
			prBssDesc->ucRCPI);
#endif
	/* 2016-01-22 Add by MEIZU BSP@YinShunQing for print WIFI scan result end */
