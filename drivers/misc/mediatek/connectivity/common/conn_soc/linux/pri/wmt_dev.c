...
static struct workqueue_struct * gPwrOnOffQueue =NULL ;
...
static void wmt_dev_early_suspend(struct early_suspend *h)
{
	...
	queue_work(gPwrOnOffQueue,&gPwrOnOffWork);
}
...
static void wmt_pwr_on_off_handler(struct work_struct *work)
{
	...
	if(wmt_core_get_any_func_state()){
		WMT_ERR_FUNC("wmt_pwr_on_off_handler GPS/BT/WIFI on\n");
		wmt_core_dump_func_state(NULL);
        return ;
	}
...
INT32 wmt_dbg_fwinfor_from_emi(INT32 par1, INT32 par2, INT32 par3)
{
	...
	if (offset == 1) {
		do {
			...
			/* 2016-2-3 Add by MEIZU BSP@YinshunQing for fixed klocwork critical issue */
#ifdef MEIZU_M80
			if (!pAddr) {
				WMT_ERR_FUNC("pAddr is NULL pointer!\n");
				break;
			}
#endif
...
#define REFRESH_TIME    300	/* sec */
	...
	/*if lcd off or stp power down,don't query temp*/
   if(wmt_dev_get_early_suspend_state()||!wmt_core_get_any_func_state()){
		WMT_INFO_FUNC("lcd or stp power down");
        return 25 ;
   	}
...
/* 2016-5-27 Modified by MEIZU BSP@YinShunQing to fixed BUG #342440 therm hold gDevWmt.psm_lock for a long time */
#ifdef MEIZU_M80
	if (query_cond) {
		...
		if (bRet == MTK_WCN_BOOL_TRUE) {
			...
			WMT_WARN_FUNC("[Thermal] enable failed!\n");
		}
...
static int WMT_init(void)
{
	...
	gPwrOnOffQueue = create_singlethread_workqueue("wmt-power") ;
#ifdef CONFIG_EARLYSUSPEND
...
static void WMT_exit(void)
{
	...
	stp_drv_exit();
	destroy_workqueue(gPwrOnOffQueue);
