...
int discoverModes(VdmManager* vdmm, SopType sop, uint16_t svid) {
	StructuredVdmHeader svdmh;
	unsigned int length = 1;
	uint32_t arr[length];
...
int processVdmMessage	(VdmManager* vdmm, SopType sop, uint32_t* arr_in, unsigned int length_in) {
	...
	unsigned int *num_sopstar_svids = NULL;
	uint16_t *svid_arr = NULL;
#if 0
			...
			case RESPONDER_ACK	:
				switch (svdmh_in.command) {
					case DISCOVER_IDENTITY	:
						...
						new_identity.has_ama_vdo = false;
						new_identity.has_cable_vdo = false;
						new_identity.has_product_vdo = false;
					...
					case DISCOVER_MODES		: 
						...
						//uint16_t* my_svids;
						//uint32_t (*my_modes)[MAX_MODES_PER_SVID];
						//svid_in = svdmh_in.svid;		// grab the SVID
						//unsigned int svid_index;
						//unsigned int* my_modes_per_svid;
						
						my_modes_info.svid 		= svdmh_in.svid;
...
int enterMode			(VdmManager* vdmm, SopType sop, uint16_t svid, unsigned int mode_index) {
	StructuredVdmHeader svdmh;
	unsigned int length = 1;	
	uint32_t arr[length];
...
int exitMode			(VdmManager* vdmm, SopType sop, uint16_t svid, unsigned int mode_index) {
	// TODO
