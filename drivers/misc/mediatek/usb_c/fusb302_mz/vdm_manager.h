 ...
 */
	...
	typedef struct {
		...
		uint16_t				sop_svids[MAX_NUM_SVIDS_PER_SOP];
		unsigned int		num_sop_svids;
		
		uint16_t				sop1_svids[MAX_NUM_SVIDS_PER_SOP];
		unsigned int		num_sop1_svids;
		
		uint16_t				sop2_svids[MAX_NUM_SVIDS_PER_SOP];
		...
		uint32_t				sop_modes [MAX_NUM_SVIDS_PER_SOP][MAX_MODES_PER_SVID];
		unsigned int		num_sop_modes_per_svid[MAX_NUM_SVIDS_PER_SOP];
		
		uint32_t				sop1_modes[MAX_NUM_SVIDS_PER_SOP][MAX_MODES_PER_SVID];
		unsigned int		num_sop1_modes_per_svid[MAX_NUM_SVIDS_PER_SOP];
		
		uint32_t				sop2_modes[MAX_NUM_SVIDS_PER_SOP][MAX_MODES_PER_SVID];
 ...
 */
	...
	int discoverModes		(VdmManager* vdmm, SopType sop, uint16_t svid);							// Discovers Modes available for each SVID
	int sendAttention		(VdmManager* vdmm, SopType sop);										// send attention command to specified sop
	int processVdmMessage	(VdmManager* vdmm, SopType sop, uint32_t* arr, unsigned int length);		// function to call when we receive VDM messages
	int enterMode			(VdmManager* vdmm, SopType sop, uint16_t svid, unsigned int mode_index);	// enter mode specified by SVID and mode index
	int exitMode			(VdmManager* vdmm, SopType sop, uint16_t svid, unsigned int mode_index);	// exit mode specified by SVID and mode index									
