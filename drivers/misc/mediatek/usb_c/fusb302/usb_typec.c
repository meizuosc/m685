	...
	if (stat == ENABLE) {
		...
		if (type == DEVICE_TYPE)
			pinctrl_select_state(typec->pinctrl,
					typec->pin_cfg->fusb340_usb18_high);
	} else
		pinctrl_select_state(typec->pinctrl,
				typec->pin_cfg->fusb340_usb18_low);
...
//edit by wpb@meizu.com END
		...
		/********************************************************/
		typec->pin_cfg->fusb340_usb18_init = pinctrl_lookup_state(typec->pinctrl, "power_en_usb18_init");
		if (IS_ERR(typec->pin_cfg->fusb340_usb18_init))
			fusb_printk(K_ERR, "Can *NOT* find power_en_usb18_init\n");
		else
			fusb_printk(K_DEBUG, "Find power_en_usb18_init\n");

		typec->pin_cfg->fusb340_usb18_low = pinctrl_lookup_state(typec->pinctrl, "power_en_usb18_low");
		if (IS_ERR(typec->pin_cfg->fusb340_usb18_low))
			fusb_printk(K_ERR, "Can *NOT* find power_en_usb18_low\n");
		else
			fusb_printk(K_DEBUG, "Find power_en_usb18_low\n");

		typec->pin_cfg->fusb340_usb18_high = pinctrl_lookup_state(typec->pinctrl, "power_en_usb18_high");
		if (IS_ERR(typec->pin_cfg->fusb340_usb18_high))
			fusb_printk(K_ERR, "Can *NOT* find power_en_usb18_init\n");
		else
			fusb_printk(K_DEBUG, "Find power_en_usb18_init\n");
		/********************************************************/
		...
		pinctrl_select_state(typec->pinctrl,
				typec->pin_cfg->fusb340_usb18_low);
		fusb_printk(K_ERR, "Finish parsing pinctrl\n");
	}
