...
#if defined(CONFIG_MZ_SMART_BATTERY)
...
{
	int bq25892_chrdet = 0, mtk_chrdet = 0;
	int chgin = 0;
	printk("[chrdet_int_handler]CHRDET status = %d....\n",
		pmic_get_register_value(PMIC_RGS_CHRDET));

#ifdef CONFIG_MTK_KERNEL_POWER_OFF_CHARGING
	bq25892_chrdet = upmu_is_chr_det();
	mtk_chrdet = upmu_get_rgs_chrdet();
	if (bq25892_chrdet == 1 || mtk_chrdet == 1)
		chgin = 1;
	else
		chgin = 0;
	if (!chgin) {
		...
		if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		    || boot_mode == LOW_POWER_OFF_CHARGING_BOOT || boot_mode == META_BOOT || boot_mode == ADVMETA_BOOT) {
			PMICLOG("[chrdet_int_handler] Unplug Charger/USB(boot mode=%d)\n", boot_mode);
			mt_power_off();
		}
	}
#endif
