			...
			if (ChoosenCluster == -1) {
				...
				if (opp[LxLL] == 0) {
					...
					if (new_state != PPM_POWER_STATE_L_ONLY)
						while (LL_limit < 4 && delta_power > delta_power_LxLL[activeCoreNumL][LL_limit+1][7]) {
					...
					if (new_state != PPM_POWER_STATE_LL_ONLY)
						while (L_limit < 4 && delta_power > delta_power_LxLL[L_limit+1][activeCoreNumLL][7]) {
			...
			for (i = 0; i < c_req->cluster_num; i++) {
				...
				if (c_req->cpu_limit[i].min_cpufreq_idx != last_req->cpu_limit[i].min_cpufreq_idx
					...
					if (c_req->cpu_limit[i].max_cpu_core != 0) {
						if (abs(max_freq - max_freq_ori) >= 5 || abs(min_freq - min_freq_ori) >= 5)
							log_print = true;
					}
				}
