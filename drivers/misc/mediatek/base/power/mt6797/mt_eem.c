...
/* FAN53200 */
unsigned int gpuFy_7_fan53200[16] = {0x2A, 0x2A, 0x27, 0x27, 0x23, 0x23, 0x20, 0x20,
			0x1C, 0x1C, 0x19, 0x19, 0x15, 0x15, 0x11, 0x11};

unsigned int gpuSb_7_fan53200[16] = {0x2C, 0x2C, 0x26, 0x26, 0x23, 0x23, 0x1F, 0x1F,
			0x1C, 0x1C, 0x19, 0x19, 0x14, 0x14, 0x11, 0x11};

unsigned int gpuMb_7_fan53200[16] = {0x2A, 0x2A, 0x26, 0x26, 0x20, 0x20, 0x1E, 0x1E,
			0x19, 0x19, 0x18, 0x18, 0x15, 0x15, 0x11, 0x11};
...
/* FAN53200 */
unsigned int gpuFy_6_fan53200[16] = {0x2A, 0x2A, 0x27, 0x27, 0x23, 0x23, 0x20, 0x20,
			0x1C, 0x1C, 0x19, 0x19, 0x15, 0x15, 0x11, 0x11};

unsigned int gpuSb_6_fan53200[16] = {0x2A, 0x2A, 0x25, 0x25, 0x22, 0x22, 0x1F, 0x1F,
			0x1B, 0x1B, 0x19, 0x19, 0x15, 0x15, 0x11, 0x11};

unsigned int gpuMb_6_fan53200[16] = {0x2A, 0x2A, 0x26, 0x26, 0x20, 0x20, 0x1E, 0x1E,
			0x19, 0x19, 0x18, 0x18, 0x15, 0x15, 0x11, 0x11};

unsigned int gpuFy_5_fan53200[16] = {0x2A, 0x2A, 0x26, 0x26, 0x20, 0x20, 0x1E, 0x1E,
			0x19, 0x19, 0x18, 0x18, 0x15, 0x15, 0x11, 0x11};

unsigned int gpuSb_5_fan53200[16] = {0x2A, 0x2A, 0x25, 0x25, 0x21, 0x21, 0x1E, 0x1E,
			0x1B, 0x1B, 0x19, 0x19, 0x14, 0x14, 0x11, 0x11};

unsigned int gpuMb_5_fan53200[16] = {0x2A, 0x2A, 0x27, 0x27, 0x22, 0x22, 0x1F, 0x1F,
			0x1B, 0x1B, 0x18, 0x18, 0x15, 0x15, 0x11, 0x11};
...
static void eem_init_det(struct eem_det *det, struct eem_devinfo *devinfo)
{
	...
	*/
	...
	case EEM_DET_GPU:
		...
		det->pmic_base =regulator_list_voltage(g_gpu_regulator, 0)/10;//convert unit from uV to 10uV
		det->pmic_step = regulator_get_linear_step(g_gpu_regulator)/10;//convert unit from uV to 10uV
		eem_info("[%s]: GPU eem_v_base=%d, eem_step=%d\n", __func__, det->pmic_base, det->pmic_step);
		...
		case CPU_UP_PREPARE:
			if ((0 == ITurboRun) && (8 == cpu) && (0 == big_cpus)) {
				...
				while ((mt_secure_call(0x8200035F, 0x10222498, 0, 0) >> 1) >
					...
					udelay(120);
					if (eem_log_en)
						eem_error("SWREQ = %x", mt_secure_call(0x8200035F, 0x10222498, 0, 0) >> 1);
				}
				udelay(120);
...
void eem_init01(void)
{
	...
	/* set non_PWM mode for RT5735 */
	regulator_set_mode(g_gpu_regulator, REGULATOR_MODE_NORMAL);/* Disable PWM mode for GPU */
...
static void eem_fixed_gpu_tables(int id)
{
	int i;
	...
	switch (id)
	{
		...
		{
			for(i=0; i<16; i++){
				gpuFy_7[i] = gpuFy_7_rt5735[i];
				gpuSb_7[i] = gpuSb_7_rt5735[i];
				gpuMb_7[i] = gpuMb_7_rt5735[i];

				gpuFy_6[i] = gpuFy_6_rt5735[i];
				gpuSb_6[i] = gpuSb_6_rt5735[i];
				gpuMb_6[i] = gpuMb_6_rt5735[i];

				gpuFy_5[i] = gpuFy_5_rt5735[i];
				gpuSb_5[i] = gpuSb_5_rt5735[i];
				gpuMb_5[i] = gpuMb_5_rt5735[i];
		...
		case EXT_FAN53200:
			for(i=0; i<16; i++){
				gpuFy_7[i] = gpuFy_7_fan53200[i];
				gpuSb_7[i] = gpuSb_7_fan53200[i];
				gpuMb_7[i] = gpuMb_7_fan53200[i];

				gpuFy_6[i] = gpuFy_6_fan53200[i];
				gpuSb_6[i] = gpuSb_6_fan53200[i];
				gpuMb_6[i] = gpuMb_6_fan53200[i];

				gpuFy_5[i] = gpuFy_5_fan53200[i];
				gpuSb_5[i] = gpuSb_5_fan53200[i];
				gpuMb_5[i] = gpuMb_5_fan53200[i];
			}
			break;
		case EXT_FAN53555:
			/*default is fan53555*/
...
static int eem_probe(struct platform_device *pdev)
{
	...
	eem_fixed_gpu_tables(vgpu_buck_id);
	///@}MEIZU_BSP
	...
	/* set PWM mode for VPU BUCK */
	regulator_set_mode(g_gpu_regulator, REGULATOR_MODE_FAST);/* Set PWM mode for GPU */
