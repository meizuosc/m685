...
#define GPU_DVFS_VOLT_MAX		(1200000)		/* uV */
...
static struct regulator *g_gpu_regulator = NULL;
static int ext_buck_id = 0;
...
static unsigned int mt_gpufreq_pmic_wrap_to_volt(unsigned int pmic_wrap_value)
{
	...
	case EXT_RT5735:
		...
		if (volt > 139375) {
			gpufreq_err("@%s: volt > 1.393v!\n", __func__);
			return 139375;
		}
		break;
	case EXT_FAN53200:
		volt = ((pmic_wrap_value * 12500) + 600000) / 10;
		if (volt > 138700) {
			gpufreq_err("@%s: volt > 1.387v!\n", __func__);
			return 138700;
		}
		break;
	case EXT_FAN53555:
...
#elif defined(VGPU_SET_BY_EXTIC)
		delay = 0;//regulator_set_voltage do the slew delay
#endif
	} else {
#ifdef VGPU_SET_BY_PMIC
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
...
#elif defined(VGPU_SET_BY_EXTIC)
	if (enable == 1) {
		if(regulator_is_enabled(g_gpu_regulator) == 0)
			ret = regulator_enable(g_gpu_regulator);
	} else {
		if(regulator_is_enabled(g_gpu_regulator) != 0)
			ret = regulator_disable(g_gpu_regulator);
	}
#endif
...
#elif defined(VGPU_SET_BY_EXTIC)
	reg_val = regulator_is_enabled(g_gpu_regulator);
#endif
	/* Error checking */
	if (enable == 1 && reg_val <= 0) {
		...
		for (i = 0; i < 10; i++) {
#ifdef VGPU_SET_BY_EXTIC
			gpufreq_err("@%s: is enabled = 0x%x\n",
				__func__, regulator_is_enabled(g_gpu_regulator));
#else
...
static void mt_gpufreq_clock_switch(unsigned int freq_new)
{
	...
	} else if (mt_gpufreq_dvfs_table_cid == 2) {
		if (freq_new >= 250000)
			mt_gpufreq_clock_switch_transient(freq_new, POST_DIV4);
		else if (freq_new >= 125000)
			mt_gpufreq_clock_switch_transient(freq_new, POST_DIV8);
...
static void mt_gpufreq_volt_switch(unsigned int volt_old, unsigned int volt_new)
{
	...
	regulator_set_voltage(g_gpu_regulator, gpu_volt*1000, GPU_DVFS_VOLT_MAX);
	gpu_volt = regulator_get_voltage(g_gpu_regulator);
	gpufreq_dbg("gpu_dvfs_set_cur_volt: request volt = %d, set volt = %d\n", volt_new, gpu_volt);
...
static unsigned int _mt_gpufreq_get_cur_volt(void)
{
	...
	gpufreq_dbg("@%s\n", __func__);
	reg_val = regulator_is_enabled(g_gpu_regulator);
	if (reg_val > 0) { /* enabled i.e. not 0mv */
		gpu_volt = regulator_get_voltage(g_gpu_regulator)/10;
...
int mt_gpufreq_ext_ic_init(void)
{
	...
	reg_val = regulator_enable(g_gpu_regulator);
#endif
	gpufreq_info("VGPU_SLEW = %d uV/us\n", 32000);
...
static struct regulator_bulk_data mt_gpu_buck_regulators[] = {
	{
		.supply = "vgpu_buck_rt5735",
		.ret = EXT_RT5735,
	},
	{
		.supply = "vgpu_buck_fan53200",
		.ret = EXT_FAN53200,
	},
	{
		.supply = "vgpu_buck_fan53555",
		.ret = EXT_FAN53555,
	},
};

struct regulator *mt_gpufreq_get_vgpu_buck(int *id)
{
	int i = 0;

	if(IS_ERR_OR_NULL(g_gpu_regulator)){
		for(i=0; i<ARRAY_SIZE(mt_gpu_buck_regulators); i++){
			g_gpu_regulator = regulator_get_optional(NULL, mt_gpu_buck_regulators[i].supply);
			if(IS_ERR(g_gpu_regulator)){
				pr_err("cannot get %s regulator\n", mt_gpu_buck_regulators[i].supply);
			}else{
				pr_info("found %s regulator\n", mt_gpu_buck_regulators[i].supply);
				*id = mt_gpu_buck_regulators[i].ret;
				break;
			}
		}
	}else{
...
static int mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	else if (mt_gpufreq_dvfs_table_type == 10)	/* 600M */
...
#ifdef VGPU_SET_BY_EXTIC
	g_gpu_regulator = mt_gpufreq_get_vgpu_buck(&buck_id);
	if(IS_ERR(g_gpu_regulator)){
		return PTR_ERR(g_gpu_regulator);
	}
	ext_buck_id = buck_id;
