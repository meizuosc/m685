...
static struct pinctrl *pinctrl_gpio;
static struct pinctrl_state *pinctrl_suspend;
static struct pinctrl_state *pinctrl_resume;
...
static int mt_gpio_probe(struct platform_device *dev)
{
	...
	pinctrl_gpio = devm_pinctrl_get(&dev->dev);
	if (IS_ERR(pinctrl_gpio)) {
		err = PTR_ERR(pinctrl_gpio);
		pr_err("Cannot find pinctrl_gpio!\n");
		return err;
	}
	pinctrl_suspend = pinctrl_lookup_state(pinctrl_gpio, "system_suspend");
	if (IS_ERR(pinctrl_suspend)) {
		err = PTR_ERR(pinctrl_suspend);
		pr_err("Cannot find pinctrl_suspend!\n");
		return err;
	}
	pinctrl_resume = pinctrl_lookup_state(pinctrl_gpio, "system_resume");
	if (IS_ERR(pinctrl_resume)) {
		err = PTR_ERR(pinctrl_resume);
		pr_err("Cannot find pinctrl_resume!\n");
		return err;
	}
	return err;
}
...
{
	int ret = 0;
	pinctrl_select_state(pinctrl_gpio, pinctrl_suspend);
	mt_gpio_suspend();
	return ret;
}
...
{
	int ret = 0;
	pinctrl_select_state(pinctrl_gpio, pinctrl_resume);
	mt_gpio_resume();
	return ret;
}
