...
#define CAM_CALERR(format, args...)    pr_info(PFX "[%s] " format, __func__, ##args)
#else
#define CAM_CALDB(x, ...)
#endif
...
#define FRONT_QTECH_MODULE   0x06
#define FRONT_SUNNY_MODULE   0x01
...
static struct i2c_board_info i2cDev2 = { I2C_BOARD_INFO(CAM_CAL_I2C_DEV2_NAME, 0xC0 >> 1)};
				  ...
				  struct i2c_client **client)
{
	...
	if (*client == NULL) {
		...
		if (adapt == NULL) {
			...
			*client = i2c_new_device(adapt, i2c_info);
...
static ssize_t sub_cam_cal_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	char *p = buf;
	bool otp_status = true;
	int *otp_data = NULL;

	if ((ov5695_qtech_otp) && (ov5695_qtech_otp->module_id == FRONT_QTECH_MODULE)) {
		otp_data = (int *)ov5695_qtech_otp;
	} else if ((ov5695_sunny_otp) && (ov5695_sunny_otp->module_id == FRONT_SUNNY_MODULE)) {
...
static ssize_t sub_cam_cal_store(struct device *dev,
				struct device_attribute *attr,
...
static struct device_attribute dev_attr_sub_ctrl = {
	.attr = {.name = "sub_cam_cal_check", .mode = 0644},
	.show = sub_cam_cal_show,
	.store = sub_cam_cal_store,
};

#define MAX_OTP_SIZE	2153
static ssize_t main_cam_cal_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	char *p = buf;
	...
	if (primax_otp_buf != NULL) {
		otp_buf = primax_otp_buf;
	} else if (sunny_otp_buf != NULL) {
		otp_buf = sunny_otp_buf;
	}
	...
	/*
	 * check whether otp data is valid or not.
	 */
	/* check AWB OTP data */
	for (i = 41; i < 62; i++) {
		if (otp_buf[i] == 0xFF)
	...
	afinf = (otp_buf[67] << 8) | otp_buf[68];
	afmid = (otp_buf[69] << 8) | otp_buf[70];
	afmacro = (otp_buf[71] << 8) | otp_buf[72];
	if (!((afinf < afmid) && (afmid < afmacro)))
		otp_status = false;

	/* check LSC&PDAF OTP data */
	j = 0;
	err_cnt = 4;
	for (i = 87; i < MAX_OTP_SIZE; i++) {
		if (otp_buf[i] == 0xFF)
			j++;
		else
			j = 0;
	...
	/*
	 * OTP info flag must be 0x01, otherwise, otp data is invalid.
	 */
	if ((otp_buf[0] != 0x01) || (otp_buf[41] != 0x01)
	    || (otp_buf[64] != 0x01) || (otp_buf[87] != 0x01)
	    || (otp_buf[1958] != 0x01))
		otp_status = false;
...
static ssize_t main_cam_cal_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	return count;
...
static struct device_attribute dev_attr_main_ctrl = {
	.attr = {.name = "main_cam_cal_check", .mode = 0644},
	.show = main_cam_cal_show,
	.store = main_cam_cal_store,
};
...
static inline int cam_cal_chrdev_register(void)
{
	...
	device_create_file(device, &dev_attr_sub_ctrl);
	device_create_file(device, &dev_attr_main_ctrl);
	meizu_sysfslink_register_name(device, "cam_cal");
