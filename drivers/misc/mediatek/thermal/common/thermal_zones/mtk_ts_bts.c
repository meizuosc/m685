...
#ifdef MEIZU_M80
#define AUX_IN2_NTC (2)     // charger ic NTC channel
...
#ifdef MEIZU_M80
static int g_RAP_ADC_channel_2 = AUX_IN2_NTC;
...
#ifdef MEIZU_M80
int get_mtk_bts_temp(void) {
	return mtkts_bts_get_hw_temp();
}
EXPORT_SYMBOL(get_mtk_bts_temp);
#endif
...
static int get_hw_charger_temp(void)
{
	...
	int ret = 0, data[4], i, ret_value = 0, ret_temp = 0, output;
	int times = 1, Channel = g_RAP_ADC_channel_2;	/* 6752=2(AUX_IN2_NTC) */
	static int valid_temp;
	...
	while (i--) {
		...
		ret += ret_temp;
		mtkts_bts_dprintk("[thermal_auxadc_get_data(AUX_IN2_NTC)]: ret_temp=%d\n",
				  ret_temp);
	}
	...
	ret = ret * 1500 / 4096;
	/* ret = ret*1800/4096;//82's ADC power */
	mtkts_bts_dprintk("Charger tery output mV = %d\n", ret);
	output = mtk_ts_bts_volt_to_temp(ret);
	mtkts_bts_dprintk("Charger output temperature = %d\n", output);
	return output;
}
...
int mtkts_charger_get_hw_temp(void)
{
	...
	if (t_ret > 45000)	/* abnormal high temp */
		pr_debug("[Power/Charger_Thermal] T_CHARGER=%d\n", t_ret);

	mtkts_bts_dprintk("[mtkts_charger_get_hw_temp] T_CHARGER, %d\n", t_ret);
	return t_ret;
}

int get_mtk_charger_temp(void) {
	return mtkts_charger_get_hw_temp();
}
EXPORT_SYMBOL(get_mtk_charger_temp);
...
int mtkts_usb_board_get_hw_temp(void)
{
	int t_ret = 0;

	mutex_lock(&BTS_lock);

	/* get HW USB Board temp (TSAP) */
	/* cat /sys/class/power_supply/AP/AP_temp */
	t_ret = get_hw_usb_board_temp();
	t_ret = t_ret * 1000;
	...
	if (t_ret > 45000)	/* abnormal high temp */
		pr_debug("[Power/Charget_Thermal] T_USB_BOARD=%d\n", t_ret);

	mtkts_bts_dprintk("[mtkts_usb_board_get_hw_temp] T_USB_BOARD, %d\n", t_ret);
	return t_ret;
}

int get_mtk_usb_board_temp(void) {
	return get_hw_usb_board_temp();
}
EXPORT_SYMBOL(get_mtk_usb_board_temp);
#endif
