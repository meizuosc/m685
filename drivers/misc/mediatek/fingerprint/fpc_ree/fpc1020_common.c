...
int fpc1020_setup_defaults(fpc1020_data_t *fpc1020)
{
	...
	#if FPC_SPI_DEBUG
	ptr = &fpc1020_setup_default_1020_a1a2;
	#endif
...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		__gpio_set_value(fpc1020->reset_gpio, 0);
		...
		if (!error) {
			...
			dev_err(&fpc1020->spi->dev, "%s timed out,retrying ... irq_gpio[%d] = %d\n",
				__func__, fpc1020->irq_gpio, __gpio_get_value(fpc1020->irq_gpio));
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
	...
	error = (__gpio_get_value(fpc1020->irq_gpio) != 0) ? -EIO : 0;
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	if (error) {
		dev_dbg(&fpc1020->spi->dev, "fpc1020_reg_access error\n");
		return error;
	}
...
int fpc1020_wait_for_irq(fpc1020_data_t *fpc1020, int timeout)
{
	...
	//dev_info(&fpc1020->spi->dev, "fpc1020_wait_for_irq timeout : %d", timeout);
	return -ETIMEDOUT;
}
		      ...
		      fpc1020_reg_access_t *reg_data)
{
	...
	memset(temp_buffer, 0, FPC1020_REG_MAX_SIZE);
