...
#define	LINK_KOBJ_NAME	"fp"
/**************************defination******************************/
#define GF_DEV_NAME "goodix_fp"
#define GF_DEV_MAJOR 0	/* assigned */

#define GF_CLASS_NAME "goodix_fp"
#define GF_INPUT_NAME "fp-keys"

#define GF_SPI_VERSION "gf_spi_tee_v0.93"

#define GF_NETLINK_ROUTE 26   /* for GF test temporary, need defined in include/uapi/linux/netlink.h */
...
// @BSP -- by zhangjiajing
	pmic_set_register_value(PMIC_RG_VIBR_VOSEL, 6);
	pmic_set_register_value(PMIC_RG_VIBR_EN, !!bonoff);
// @BSP --
#else	if (bonoff)
		hwPowerOn(MT6351_POWER_LDO_VIBR, VOL_2800, "fingerprint");
	else
		hwPowerDown(MT6351_POWER_LDO_VIBR, "fingerprint");
#endif
}
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if (GF_KEY_HOME == gf_key.key) {
			key_event = GF_INPUT_BACK_KEY;//GF_INPUT_HOME_KEY;
...
unsigned char str_chipid[64] = "GFx16M";
static ssize_t gf_show_chip_id(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(chip_id, S_IRUGO, gf_show_chip_id, NULL);
...
static u8 gf_get_vendor_id(struct gf_dev *gf_dev)
{
	u32 time_out = 0;
	u8 tmp_buf[32] = {0};

	// 1
	gf_spi_write_byte_ree(0x8041, 0x08);
	gf_spi_write_byte_ree(0x8042, 0xF8);
	...
	do
	{
		gf_spi_read_bytes_ree(0x8042, 1, tmp_buf);
		if(tmp_buf[0] == 0xAA )
			break;
	}while(time_out++ < 600);
	gf_debug(INFO_LOG, "%s, flash mode ready, time_out=%d\n", __func__, time_out);

	// 3 	
	gf_spi_write_byte_ree(0x5094, 0x0C);
	// 4
	time_out = 0;
	do
	{
		gf_spi_read_bytes_ree(0x5094, 1, tmp_buf);
		if(tmp_buf[0] == 0x00 )
			break;
	}while(time_out++ < 600);
	gf_debug(INFO_LOG, "%s, vendor data ready, time_out=%d\n", __func__, time_out);	

	// 5
	gf_spi_read_bytes_ree(0x8148, 1, tmp_buf);
	gf_debug(INFO_LOG, "[%s],  the vendor id is 0x%.02X.\n", __func__, tmp_buf[0]);
	
	//6
	gf_spi_write_byte_ree(0x8041, 0x00);
...
//	mdelay(80);
	...
	vendorID = gf_get_vendor_id(gf_dev);
	sprintf(str_chipid,"%s-%.2X-%.2X%.2X%.2X",tmp_buf,vendorID,tmp_buf[7], tmp_buf[8], tmp_buf[9]);
...
/*-------------------------------------------------------------------------*/
// @BSP -- by zhangjiajing
extern int fingerprint_is_fpc(void);
// @BSP --
...
#ifdef CONFIG_FINGERPRINT_FPC	
// @BSP -- by zhangjiajing
	if (fingerprint_is_fpc()) {
		printk(KERN_EMERG "[gf] " "fpc probed, not goodix fingerprint\n");
		FUNC_EXIT();
		return -EINVAL;
	}
// @BSP --
	...
	status = device_create_file(&spi->dev,&dev_attr_chip_id);
	if (status) {
		gf_debug(ERR_LOG, "%s, Failed to create chip_id sysfs file.\n", __func__);
		status = -ENODEV;
	} else {
		gf_debug(INFO_LOG, "%s, Success create chip_id sysfs file.\n", __func__);
	}
	
	meizu_sysfslink_register_name(&spi->dev,LINK_KOBJ_NAME);
...
err_cdev:
	...
	device_remove_file(&spi->dev, &dev_attr_chip_id);
