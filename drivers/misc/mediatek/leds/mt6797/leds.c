...
#else
#if defined(CONFIG_BACKLIGHT_SUPPORT_LM36272)
					pled_dtsi[i].data =
					    (long)lm36272_set_brightness;
#else
					pled_dtsi[i].data =
				  ...
				  struct nled_setting *led, int enable)
{
	/* int time_index = 0; */
	/* int duty = 0; */
#ifdef MEIZU_M80 /* 2015-10-29 add by Yin ShunQing, for led breath mode */
...
#ifdef MEIZU_M80 /* 2015-11-24 add by Yin ShunQing, for light sensor driver invoke */
void turn_off_led_isink0(void)
{
	...
	if (*delay_on != led_data->delay_on
		...
		if (led_data->delay_on && led_data->delay_off) {	/* enable blink */
			...
			if (led_data->cust.mode == MT65XX_LED_MODE_PWM) {
				...
				nled_tmp_setting.blink_on_time =
				    led_data->delay_on;
#ifndef MEIZU_M80 /* 2015-10-29 add by Yin ShunQing, for led breath mode */
				mt_led_blink_pmic(led_data->cust.data,
						  &nled_tmp_setting);
#else
				/* set led isink0: breath mode */
				led_switch_breath_pmic(led_data->cust.data,
						&nled_tmp_setting, ISINK_BREATH_MODE);
				//clear delay_on and delay_off
				led_data->delay_on = 0;
				led_data->delay_off = 0;
#endif /* end #ifndef MEIZU_M80 */
				return 0;
