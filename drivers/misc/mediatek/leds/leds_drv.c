...
******************************************************************************/
#ifdef CONFIG_BACKLIGHT_SUPPORT_LM36272
...
int setMaxbrightness(int max_level, int enable)
{
	...
	disp_bls_set_max_backlight(((((1 << MT_LED_INTERNAL_LEVEL_BIT_CNT) -
				      1) * max_level + 127) / 255));
...
{
        /* 2015-10-29 add by Yin ShunQing, for led breath mode */
#ifdef MEIZU_M80
        struct mt65xx_led_data *led_data =
                container_of(led_cdev, struct mt65xx_led_data, cdev);
...
int backlight_brightness_set(int level)
{
	...
	if (MT65XX_LED_MODE_CUST_BLS_PWM ==
		...
		return mt65xx_led_set_cust(&cust_led_list[MT65XX_LED_TYPE_LCD],
					   ( level << (LED_INTERNAL_LEVEL_BIT_CNT - MT_LED_INTERNAL_LEVEL_BIT_CNT)));
#else
		return mt65xx_led_set_cust(&cust_led_list[MT65XX_LED_TYPE_LCD],
					   (level >>
					    (MT_LED_INTERNAL_LEVEL_BIT_CNT -
...
static ssize_t show_default_brightness(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%u\n", 50);
}
static DEVICE_ATTR(default_brightness, 0444, show_default_brightness, NULL);
/****************************************************************************
 * driver functions
 ***************************************************************************/
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
			meizu_sysfslink_register_name(g_leds_data[i]->cdev.dev, "lcd-backlight");
			ret = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_default_brightness);
			if (ret)
				goto err;
		}
	}
