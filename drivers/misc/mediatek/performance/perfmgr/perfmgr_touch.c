			    ...
			    unsigned int code, int value)
{
	...
	if ((type == EV_KEY) && (value == 1)) {
		if (code == BTN_TOUCH || code == KEY_POWER) {
			pr_debug("dbs_input_event: type:%d, code:%d, value:%d\n", type, code, value);
			spin_lock_irqsave(&tboost.touch_lock, flags);
...
int perfmgr_touch_suspend(void)
{
	...
	unsigned long flags;
	spin_lock_irqsave(&tboost.touch_lock, flags);
	perf_mgr_touch_enable = 0;
	spin_unlock_irqrestore(&tboost.touch_lock, flags);
	return 0;
}

int perfmgr_touch_resume(void)
{
	/*pr_debug(TAG"perfmgr_touch_resume\n");*/
	unsigned long flags;
	spin_lock_irqsave(&tboost.touch_lock, flags);
	perf_mgr_touch_enable = 1;
	spin_unlock_irqrestore(&tboost.touch_lock, flags);
	return 0;
}
