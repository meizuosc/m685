		...
		if (VoiceData_Wait_Queue_flag == 1) {
			...
			if ((VowDrv_GetHWStatus() == VOW_PWR_OFF) || (vowserv.recording_flag == false)) {
				...
				/*PRINTK_VOWDRV("TX Leng:%d, %d, %d\n",
						...
						vowserv.voice_length);*/
				if (vowserv.voice_length > (VOW_VOICE_RECORD_THRESHOLD >> 1))
					PRINTK_VOWDRV("PCM DUMP ERR, VOICE_LENGTH WRONG\n");
				if (vowserv.voice_buf_offset > (VOW_VOICE_RECORD_THRESHOLD >> 1))
					PRINTK_VOWDRV("PCM DUMP ERR, VOICE_OFFSET WRONG\n");
				if (vowserv.voicedata_idx > (VOW_VOICE_RECORD_THRESHOLD >> 2))
					PRINTK_VOWDRV("PCM DUMP ERR, VOICE_INDEX WRONG\n");
			#endif
