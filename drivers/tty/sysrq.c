...
static void sysrq_switch_sysrq(struct work_struct *work)
{
	musb_uart_usb_switch(0xff);
...
static void sysrq_do_reset(unsigned long _state)
{
	...
	queue_work(system_highpri_wq, &state->switch_work);
}
...
#ifdef CONFIG_MTK_UART_USB_SWITCH
	INIT_WORK(&sysrq->switch_work, sysrq_switch_sysrq);
#endif
...
///{@MEIZU_BSP
#ifndef CONFIG_MTK_UART_USB_SWITCH
