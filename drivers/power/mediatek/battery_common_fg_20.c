...
#ifdef MEIZU_M80
#define TELECOM_VERSION (0X1 << 3)
#define B2_VERSION  (0X3)
static kal_bool ti_charger_flag = KAL_TRUE;
#endif
...
static struct notifier_block chg_fb_notifier;
#endif
...
#ifdef MEIZU_M80
void set_bat_thread_priority(unsigned short pri_val)
{
        ...
        const union power_supply_propval *val)
{
 //   struct battery_data *data = container_of(psy, struct battery_data, psy);
    ...
    case POWER_SUPPLY_PROP_MANUFACTURER:
        BMT_status.manufacturer_name = strcat(battery_pack, val->strval);
        break;
...
#ifdef MEIZU_M80
static ssize_t show_usb_temp(struct device *dev,struct device_attribute *attr,
					char *buf)
{
	int temp;

	temp = get_mtk_usb_board_temp() / 1000;

	return sprintf(buf, "%d\n", temp);
}

static ssize_t store_usb_temp(struct device *dev,struct device_attribute *attr,
		const char *buf, size_t size)
{

	return size;
}
static DEVICE_ATTR(usb_temp, 0664, show_usb_temp, store_usb_temp);
...
/* add by lyf for 3rd_fg begin */
#ifdef MEIZU_M80
    #if defined(SOC_BY_3RD_FG)
    //battery_meter_set_3rd_fg_temp(BMT_status.temperature);
    BMT_status.UI_SOC2 = battery_meter_get_3rd_fg_soc();
    if ((BMT_status.UI_SOC2 < 0) ||(BMT_status.UI_SOC2 >100)) {
        BMT_status.UI_SOC2 = 50;
    }
    #endif
#endif
/* add by lyf for 3rd_fg end */
...
{
	char *battery_cell;
	int ap_board_temperature,charger_temperature,usb_board_temperature;

	if (BMT_status.temperature == ERR_CHARGE_TEMPERATURE) {
		return PMU_STATUS_FAIL;
	}

	ap_board_temperature = get_mtk_bts_temp() / 1000;
	charger_temperature = get_mtk_charger_temp() / 1000;
	usb_board_temperature = get_mtk_usb_board_temp() / 1000;

            if(charger_temperature >= T70_CHARGE_TEMPERATURE || usb_board_temperature >= T70_CHARGE_TEMPERATURE){
                printk("[BATTERY] Charger or USB board  Over Temperature !!\n\r");
	...
	/*
		ATL cell:
		0<=    T  <10'c                         0.3C CC to 4.35V, CV to 0.05C
		10<= T <25'c                        1.5C CC to 4.2V, 0.5C CC to 4.35V, CV to 0.05C.
		25<= T <= 44'C	                    1.5C CC to 4.35V. CV to 0.05C
		44<    T <= 45'C 	            0.5C CC to 4.35V, CV to 0.05C
	*/
	if (!strcmp(battery_cell,"ATL")) {
		if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
		...
		} else if (BMT_status.temperature < T25_CHARGE_TEMPERATURE) {
			printk("[BATTERY] Battery Temperature Under temperature 25oC\n\r");
			g_batt_temp_status = TEMP_POS_T25;
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		} else if (BMT_status.temperature <= T45_CHARGE_TEMPERATURE) {
			printk("[BATTERY] Battery Temperature Under Temperature 45oC!!,ap_board_temperature %d\n\r",
					ap_board_temperature);
			/*
			if (g_batt_temp_status == TEMP_POS_T42 || g_batt_temp_status == TEMP_POS_T55) {
				if (max(ap_board_temperature, BMT_status.temperature) < T38_CHARGE_TEMPERATURE) {
					g_batt_temp_status = TEMP_POS_NORMAL;
				}
			} else {
				g_batt_temp_status = TEMP_POS_NORMAL;
			}
			if (max(ap_board_temperature, BMT_status.temperature) >= T42_CHARGE_TEMPERATURE) {
			...
			*/
			...
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		} else if (BMT_status.temperature <= T55_CHARGE_TEMPERATURE) {
			printk("[BATTERY] Battery Temperature Under Temperature 55oC!!\n\r");
			g_batt_temp_status = TEMP_POS_T55;
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		} else if (BMT_status.temperature > T55_CHARGE_TEMPERATURE){
	...
	/*
		SONY cell:
		0<=    T  <10'c               0.3C CC to 4.35V, CV to 0.05C
		10<= T <15'c 	            1.0C CC to 4.35V, CV to 0.05C.
		15<= T <= 44'C		1.5C CC to 4.35V. CV to 0.05C
		44<    T <= 45'C 	0.5C CC to 4.35V, CV to 0.05C
	*/
		if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
			printk("[BATTERY] Battery Under 0OC Temperature or NTC fail !!\n\r");
...
{
   *(unsigned int*)(data) = g_batt_temp_status;
}
...
#else
    #ifdef MEIZU_M80
	ret_val = bq27532_battery_voltage();
    #else
...
void mt_battery_GetBatteryData(void)
{
    #ifdef MEIZU_M80
            unsigned int curr = 0;
            signed int ICharging = 0,temperature = 25;
	...
	static signed int previous_SOC = -1;
        ...
        if (temperature_debug == KAL_FALSE)
            ...
            BMT_status.charger_vol = bq27532_charger_voltage();
...
void do_chrdet_int_task(void)
{
	...
	mt_battery_disable_charging_algorithm();
#endif
...
void BAT_thread(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
		mt_battery_CheckBatteryStatus();
                #ifdef MEIZU_M80
                    //Just for compatible fairchild and TI
                    if(ti_charger_flag){
...
#ifdef MEIZU_M80
//Accord hardware version:
//TODO: need by hujian interface
extern int get_hw_version(void);
void charger_ic_is_ti(void)
{
    int hw_version;

    hw_version = get_hw_version();
    hw_version &= (~TELECOM_VERSION);

    pr_debug("charger_ic_is_ti: hw_version = 0x%x\n",hw_version);

    if(hw_version >= B2_VERSION)   // ti chager ic
        ti_charger_flag = KAL_TRUE;
    else
        ti_charger_flag = KAL_FALSE;    //fairchild charger ic
}
...
static int battery_probe(struct platform_device *dev)
{
...
#endif
#ifdef MEIZU_M80
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_usb_temp);
    ...
    #ifdef MEIZU_M80
	bat_thread_task = kthread_create(bat_routine_thread, NULL, "bat_routine_thread");
...
#ifdef MEIZU_M80
	chg_fb_notifier.notifier_call = fb_event_notify;
	ret = fb_register_client(&chg_fb_notifier);
	if(ret)
...
#ifdef MEIZU_M80
unsigned int Get_charger_status(void)
{
	if (BMT_status.charger_exist == KAL_TRUE)
	{
		return 1;
...
static void battery_timer_resume(void)
{
	...
	if (g_call_state == CALL_ACTIVE
		...
		BMT_status.UI_SOC2 = BMT_status.UI_SOC;
                #else
