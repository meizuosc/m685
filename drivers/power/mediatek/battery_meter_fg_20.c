...
#else
    #ifdef MEIZU_M80
            return bq27532_battery_read_temperature();
    #else
...
signed int battery_meter_get_charger_voltage(void)
{
	...
	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_CHARGER, &val);
	 ...
	 //val = (((R_CHARGER_1+R_CHARGER_2)*100*val)/R_CHARGER_2)/100; 
            printk(KERN_EMERG "battery_meter_get_charger_voltage:    the charger vol is : %d \n",val);
	return val;
}

signed int battery_meter_get_3rd_fg_soc(void)
{
        int val = 0;

        battery_meter_ctrl(BATTERY_METER_CMD_GET_FG_SOC, &val);
