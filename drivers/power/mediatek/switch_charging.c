...
/* ============================================================ // */
#ifdef MEIZU_M80
unsigned int g_changed_cv = 0;
static kal_bool cv_changed_flag = KAL_FALSE;
bool g_enable_sub_charger_flag = false;
#endif
...
/* ============================================================ // */
#ifdef MEIZU_M80
static unsigned int batt_temp_status = TEMP_POS_NORMAL;
...
#ifdef MEIZU_M80
	battery_charging_control(CHARGING_CMD_SET_ONLYONE_INPUT_CURRENT,&chr_current);
#else
...
static void mtk_pe20_increase(void)
{
	kal_bool ta_20_current_pattern = KAL_TRUE;	/* TRUE = increase */

	if (ta_cable_out_occur == KAL_FALSE) {
		battery_charging_control(CHARGING_CMD_SET_TA20_CURRENT_PATTERN, &ta_20_current_pattern);
	} else {
		ta_check_chr_type = KAL_TRUE;
		battery_log(BAT_LOG_CRTI, "[PE+20]mtk_ta_increase() Cable out\n");
	}
}

#define PE20_RETRY_TIMES 5
#define PE20_SW_RETRY_TIMES 5
static kal_bool mtk_pe20_retry_increase(void)
{
	int real_v_chrA;
	int real_v_chrB;
...
*/
	do {
		...
		mtk_pe20_increase();	/* increase TA voltage to 7V */
		...
		if (real_v_chrB - real_v_chrA >= 1000) /* 1.0V */
			...
			retransmit_count++;
			battery_log(BAT_LOG_CRTI,
				    "[PE+20]Communicated with adapter:retransmit_count =%d, chrA=%d, chrB=%d\n",
	...
	battery_log(BAT_LOG_CRTI,
		    "[PE+20]Finished communicating with adapter real_v_chrA=%d, real_v_chrB=%d, retry=%d\n",
		    real_v_chrA, real_v_chrB, retransmit_count);

	if (retransmit_count == PE20_SW_RETRY_TIMES)
		return KAL_FALSE;
...
static void mtk_pe20_detector(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[PE+20]starting PE+ 2.0 adapter detection\n");

	pe20_v_chr_org = battery_meter_get_charger_voltage();
	 mtk_pe20_retry_increase();
	real_v_chrB = battery_meter_get_charger_voltage();

	if (real_v_chrB - pe20_v_chr_org >= 1000 || real_v_chrB >= 6000){
		is_pe20_connect = KAL_TRUE;
	}
	else
		is_pe20_connect = KAL_FALSE;

	battery_log(BAT_LOG_CRTI, "[PE+20]End of PE+ adapter detection, is_pe20_connect=%d\n",
		    is_pe20_connect);
}
...
static void battery_pump_express20_algorithm_start(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		"[PE+20]start vbus_vol:%d bat_vol:%d current_sign:%d charging_current:%d \n"
		, vbus_vol,bat_vol,(int)current_sign, (int)charging_current);
...
void set_pe20_charging_current(void)
{
	...
	g_temp_input_CC_value = PE20_AC_7_5V_INPUT_CURRENT;
	g_temp_CC_value = PE20_AC_CHARGING_CURRENT;
	g_enable_sub_charger_flag = true;
}
#endif
...
#ifdef MEIZU_M80
	if (real_v_chrB - ta_v_chr_org >= 1000 || real_v_chrB >= 6000)
#else
...
#ifdef MEIZU_M80
        battery_log(BAT_LOG_CRTI,
			    "[PE+]Stop battery_pump_express_charger_check, UI_SOC2=%d, ta_check_chr_type = %d, charger_type = %d\n",
			    BMT_status.UI_SOC2, ta_check_chr_type, BMT_status.charger_type);
#else
        battery_log(BAT_LOG_CRTI,
...
static void battery_pump_express_algorithm_start(void)
{
    ...
    #ifdef MEIZU_M80
		} else if (BMT_status.UI_SOC2 > batt_cust_data.ta_stop_battery_soc) {
    #else
		} else if (BMT_status.UI_SOC2 > batt_cust_data.ta_stop_battery_soc) {
    #endif
...
*/
	...
	switch (batt_temp_status) {
		case TEMP_POS_T10:
			...
			g_enable_sub_charger_flag = false;
			break;

		case TEMP_POS_T15:
 			if(is_pe20_connect == KAL_TRUE) {
				g_temp_CC_value = PE20_AC_1C_CHARGING_CURRENT;
				g_temp_input_CC_value = PE20_AC_1C_INPUT_CURRENT;
				g_enable_sub_charger_flag = true;
			}
 			else if(is_ta_connect == KAL_TRUE) {
				g_temp_CC_value = TA_AC_1C_CHARGING_CURRENT;
				g_temp_input_CC_value = TA_AC_1C_INPUT_CURRENT;
				g_enable_sub_charger_flag = true;
			}
			else {
		...
		case TEMP_POS_T25:
			if ((is_ta_connect == KAL_TRUE) && g_changed_cv == 0) {
				g_temp_CC_value = TA_AC_1_5C_CHARGING_CURRENT;//9V
				g_temp_input_CC_value = TA_AC_1_5C_INPUT_CURRENT;
				cv_voltage = BATTERY_VOLT_04_200000_V;
				g_enable_sub_charger_flag = true;
			}
			else if ((is_pe20_connect == KAL_TRUE) && g_changed_cv == 0) {
				g_temp_CC_value = PE20_AC_1_5C_CHARGING_CURRENT;//9V
				g_temp_input_CC_value = PE20_AC_1_5C_INPUT_CURRENT;
				cv_voltage = BATTERY_VOLT_04_200000_V;
				g_enable_sub_charger_flag = true;
			}
			else {
...
{
	battery_log(BAT_LOG_CRTI, "select_charging_current start....\r\n");
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
	...
	#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
		if (batt_temp_status == TEMP_POS_NORMAL || (batt_temp_status == TEMP_POS_T25
				&& g_changed_cv == 0)
		...
		if (get_usb_current_unlimited()) {
			...
			battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_voltage);
			printk("[BATTERY] select_charging_curret, batt_temp_status %d, batt_temp_pre_status %d!\n",
					batt_temp_status, batt_temp_pre_status);
		}
	...
	#ifdef MEIZU_M80
	if (KAL_TRUE == is_ta_connect ||KAL_TRUE == is_pe20_connect){
		if(g_enable_sub_charger_flag && !BMT_status.is_screen_on)
			battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
		else
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			battery_log(BAT_LOG_FULL,
				    "USB_CURRENT_UNLIMITED, use batt_cust_data.ac_charger_current\n");
#ifndef CONFIG_MTK_SWITCH_INPUT_OUTPUT_CURRENT_SUPPORT   // no
        ...
        void *data)
{
	...
	case FB_BLANK_POWERDOWN:	//lcd power off
		...
		hiz_enable = KAL_FALSE;
		battery_charging_control(CHARGING_CMD_SET_HIZ_SUB, &hiz_enable);
		break;
...
{
	return BMT_status.is_screen_on;
}
...
static void mt_bcct_check(void)
{
	...
	if (batt_temp_status == TEMP_POS_T25) {
	...
	if ((batt_temp_pre_status != batt_temp_status) &&
			(batt_temp_status == TEMP_POS_NORMAL ||
			 (batt_temp_status == TEMP_POS_T25 && g_changed_cv == 0)
			|| batt_temp_status == TEMP_POS_T15)) {
		 is_ta_connect = KAL_FALSE;
		 ta_check_chr_type = KAL_TRUE;
		 ta_cable_out_occur = KAL_FALSE;
...
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT)
	#ifdef MEIZU_M80
	if ((cmd_discharging == -1) && (batt_temp_status == TEMP_POS_NORMAL || (batt_temp_status == TEMP_POS_T25
				&& g_changed_cv == 0)
			|| batt_temp_status == TEMP_POS_T15))
	#endif
...
#ifdef MEIZU_M80
void mt_battery_disable_charging_algorithm(void)
{
	unsigned int charging_enable;
