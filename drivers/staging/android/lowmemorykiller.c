	...
	*/
	...
	for (i = 0; i < array_size; i++) {
		...
		if ((sc->gfp_mask & (__GFP_MOVABLE|__GFP_HIGHMEM)) != (__GFP_MOVABLE|__GFP_HIGHMEM)) {
			...
			if (zone_free <= min_wmark_pages(zone))
				{
				//lowmem_print(2, "[LMK] 1 zone_free=%ld, zone_file=%ld, min_wmark=%ld\n",zone_free,zone_file,min_wmark_pages(zone));
			...
			if (zone_free <= low_wmark_pages(zone) && zone_file <= SWAP_CLUSTER_MAX)
				{
				//lowmem_print(2, "[LMK] 2 zone_free=%ld, zone_file=%ld, low_wmark=%ld, SWAP_CLUSTER_MAX=%ld\n",zone_free,zone_file,low_wmark_pages(zone),SWAP_CLUSTER_MAX);
				should_be_aggressive = true;
				}

			/* To be aggressive if needed */
			if (should_be_aggressive) {
				//lowmem_print(1, "Aggressive LMK for severe low memory!\n");
				min_score_adj = 0;
			}
		}
#endif
	...
	for_each_process(tsk) {
		...
		if (p->signal->flags & SIGNAL_GROUP_COREDUMP) {
			lowmem_print(1, "lowmem_scan filter process: %d (%s) is in middle of coredump, ignore it.", p->pid, p->comm);
			task_unlock(p);
			continue;
		}
#endif
		if (p->state & TASK_UNINTERRUPTIBLE) {
			lowmem_print(1, "lowmem_scan filter D state process: %d (%s) state:0x%lx\n", p->pid, p->comm, p->state);
			task_unlock(p);
			continue;
		}
