...
#define STYLUS_DRIVER_NAME "synaptics_dsx/stylus_dri"
...
static ssize_t synaptics_rmi4_appid_show(struct device *dev,
				struct device_attribute *attr, char *buf);
...
#ifdef MEIZU_M80
static ssize_t synaptics_rmi4_set_SleepBit(struct device *dev, struct device_attribute *attr, const char *buf, size_t size);
static ssize_t synaptics_rmi4_get_SleepBit(struct device *dev, struct device_attribute *attr, char *buf);
#endif
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_CUST_GESTURE
...
#endif
#ifdef MEIZU_M80
     __ATTR(sleep_set, (S_IRUGO|S_IWUSR|S_IWGRP),
					synaptics_rmi4_get_SleepBit,
					synaptics_rmi4_set_SleepBit),
#endif
};
				...
				struct device_attribute *attr, char *buf)
{
	...
	count += sprintf(tmp + count, "M80SEC");

	if((mz_system_lcd_id & 0x01) == 0x01)
		count += sprintf(tmp + count, "W");
	else
		count += sprintf(tmp + count, "B");

	count += sprintf(tmp + count, ":%d-%s:Synaptics:%X\n",
			rmi4_data->firmware_id, rmi4_data->config_id, mz_system_lcd_id);
		...
		struct synaptics_rmi4_fn *fhandler)
{
...
#ifndef CONFIG_TOUCHSCREEN_SYNAPTICS_CUST_GESTURE
	unsigned char detected_gestures;
#else
...
#endif
	static unsigned char fstatus[10];
...
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_CUST_GESTURE
	if (rmi4_data->suspend && rmi4_data->gesture_enable) {
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		if (finger_status != fstatus[finger])
			printk("finger %d %s", finger, finger_status == 0 ? "up" : "down");
		...
		default:
			...
			input_mt_report_slot_state(rmi4_data->input_dev,
					MT_TOOL_FINGER, 0);
			//input_report_abs(rmi4_data->input_dev, ABS_MT_PRESSURE, 0); //meizu
#endif
			break;
		}
		if (finger_status != fstatus[finger]) {
			printk("\n");
			fstatus[finger] = finger_status;
		}
	}
...
#ifdef MEIZU_M80_TP_CHARGE
typedef enum {
	KAL_FALSE = 0,
	KAL_TRUE  = 1,
} kal_bool;
#define CHARGERBIT (1<<5)
extern unsigned int Get_charger_status(void);
extern unsigned int charge_reflash_flag;
int is_ChargerBit_set = 0;

static void synaptics_rmi4_set_ChargerBit(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	unsigned char device_ctrl;
	unsigned int WithCharger = 0;
	//struct synaptics_rmi4_data *rmi4_data = exp_data.rmi4_data;

	if (charge_reflash_flag == 1)
	{
		dev_err(rmi4_data->pdev->dev.parent,"%s firmware is reflashing \n",__func__);
		return;
	}

	WithCharger = Get_charger_status();
	if ((is_ChargerBit_set == 1) && (WithCharger == 1))
	{
		dev_dbg(rmi4_data->pdev->dev.parent,"%s Charger in ChargerBit is already set \n",__func__);
		return;
	}

	if ((is_ChargerBit_set == 0) && (WithCharger == 0))
	{
		dev_dbg(rmi4_data->pdev->dev.parent,"%s Charger out ChargerBit is already set \n",__func__);
		return;
	}
	...
	if (retval < 0)
	{
		dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to set ChargerBit\n",__func__);
		return;
	}
	if (WithCharger)
	{
...
static ssize_t synaptics_rmi4_get_SleepBit(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (retval < 0)
	{
		dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to set SleepBit\n",__func__);
		return -1;
	}
	return sprintf(buf, "%d\n", (device_ctrl & SLEEPBIT));
}
...
extern void tapdet_update_input_event_time(void);
#endif
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	if (gpio_get_value(bdata->irq_gpio) != bdata->irq_on_state)
		goto exit;
#ifdef MEIZU_M80_TP_CHARGE
	synaptics_rmi4_set_ChargerBit(rmi4_data);
#endif
	synaptics_rmi4_sensor_report(rmi4_data, true);

#ifdef CONFIG_SND_SOC_TFA98XX
	tapdet_update_input_event_time();
#endif
exit:
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (bdata->power_gpio >= 0) {
		...
		gpio_set_value(bdata->power33_gpio, bdata->power_on_state);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	*/
	...
	retval = sysfs_create_link(rmi4_data->devices,rmi4_data->sensor00, MZ_KOBJ_NAME);
	retval = sysfs_create_link(rmi4_data->devices,rmi4_data->sensor00, SYNAPICS_KOBJ_NAME);
	retval = meizu_sysfslink_register_name(&rmi4_data->input_dev->dev, "tp");
	if (retval < 0)
	{
		dev_err(&rmi4_data->input_dev->dev, "%s: could not create meizu sysfs files\n",__func__);
	}
#endif
#ifdef CONFIG_FB
