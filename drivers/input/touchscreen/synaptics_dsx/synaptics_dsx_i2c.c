...
#ifdef CONFIG_OF
static struct of_device_id synaptics_of_match[] = {
	{
...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	np = of_find_matching_node(np, synaptics_of_match);
	if (np == NULL) {
		dev_err(dev, "can not find touch data in dts\n");
		return -EINVAL;
	}
	...
	if (of_property_read_bool(np, "synaptics,power-gpio")) {
		...
		bdata->power33_gpio = of_get_named_gpio_flags(np,
				"synaptics,power33-gpio", 0, NULL);
