...
#define FW_IMAGE_NAME_V0 "synaptics/startup_fw_update_v0.img"
#define FW_IMAGE_NAME_V2 "synaptics/startup_fw_update_v2.img"
//#define FW_IMAGE_NAME_V3 "synaptics/startup_fw_update_v3.img"
...
#define FB_READY_TIMEOUT_S 600//30
#endif
#endif
...
#define WRITE_SIZE_LIMIT 2048

#define TP_CUST_SER_V2 0x3032
...
static int fwu_get_device_config_id(void)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				fwu->config_id,
				config_id_size);
	strcpy(rmi4_data->config_id, fwu->config_id);
	if (retval < 0)
		return retval;
...
#ifdef MEIZU_M80
	//if ((flash_area == UI_CONFIG) || (flash_area == UI_FIRMWARE))
	//{
		retval = strcmp(fwu->config_id,"V003-M80-SDC");
		if (retval < 0)
		{
			fwu->bl_mode_device = true;// Do not copy calibration
			printk("%s config id is V3 (%s)\n", __func__, fwu->config_id);
		}
		else
		{
			printk("%s config id is other (%s)\n", __func__, fwu->config_id);
		}
//	}
#endif
		dev_info(rmi4_data->pdev->dev.parent,
...
static int fwu_erase_utility_parameter(void)
{
	...
	if (retval < 0)
	{
		printk("%s fwu_write_f34_command CMD_ERASE_UTILITY_PARAMETER error\n",__func__);
		return retval;
	}
	...
	if (retval < 0)
	{
		printk("%s fwu_wait_for_idle ERASE_WAIT_MS error\n",__func__);
		return retval;
	}
...
static int fwu_write_bl_area_v7(void)
{
	...
	if (fwu->has_utility_param) {
		...
		if (retval < 0)
		{
			printk("%s fwu_erase_utility_parameter error\n",__func__);
			return retval;
		}
	}
	...
	if (retval < 0)
	{
		printk("%s fwu_erase_configuration 1 error\n",__func__);
		return retval;
	}
	...
	if (retval < 0)
	{
		printk("%s fwu_erase_configuration 2 error\n",__func__);
		return retval;
	}
	...
	if (retval < 0)
	{
		printk("%s fwu_erase_bootloader error\n",__func__);
		return retval;
	}
	...
	if (retval < 0)
	{
		printk("%s fwu_write_bootloader error\n",__func__);
		return retval;
	}
	...
	if (retval < 0)
	{
		printk("%s fwu_write_configuration 1 error\n",__func__);
		return retval;
	}
	rmi4_data->reset_device(rmi4_data, false);
	...
	if (retval < 0)
	{
		printk("%s fwu_write_configuration 2 error\n",__func__);
		return retval;
	}
	...
	if (fwu->img.contains_utility_param) {
		...
		if (retval < 0)
		{
			printk("%s fwu_write_utility_parameter error\n",__func__);
			return retval;
		}
	}
...
static int fwu_do_reflash(void)
{
	...
	if (do_bl_update) {
		printk("%s do_bl_update enter\n",__func__);
		retval = fwu_write_bl_area_v7();
		if (retval < 0)
		{
			printk("%s fwu_write_bl_area_v7 error \n",__func__);
			return retval;
		}
...
unsigned int synaptics_fw_get_customer_serialization(void)
{
	unsigned int retval;

	if (!fwu)
	{
		printk("%s: fwu is NULL\n", __func__);
		return -ENODEV;
	}
	fwu->config_area=PM_CONFIG_AREA;
	...
	if (retval < 0)
		{
		printk("%s: Failed to read config\n", __func__);
		return retval;
		}
		/*
	printk("%s: buf0 = %x, buf1 = %x, buf2 = %x, buf3 = %x, buf4 = %x, buf5 = %x, buf6 = %x \n",
		__func__, fwu->read_config_buf[0], fwu->read_config_buf[1], fwu->read_config_buf[2], fwu->read_config_buf[3],
		fwu->read_config_buf[4], fwu->read_config_buf[5], fwu->read_config_buf[6]);
	*/
	retval = ((fwu->read_config_buf[5]<<8) | fwu->read_config_buf[6]);
	return retval;
}
unsigned int charge_reflash_flag = 0;
static int fwu_start_reflash(void)
{
	...
	rmi4_data->stay_awake = true;
	cust_ser = synaptics_fw_get_customer_serialization();
	mutex_lock(&rmi4_data->rmi4_exp_init_mutex);
	charge_reflash_flag = 1;
	pr_notice("%s: Start of reflash process cust_ser = %x \n", __func__, cust_ser);

	if (fwu->image == NULL) 
	{
		if (cust_ser >= TP_CUST_SER_V2)
		{
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,FW_IMAGE_NAME_V2, sizeof(FW_IMAGE_NAME_V2),sizeof(FW_IMAGE_NAME_V2));
			//fwu->force_update = true;
			//goto exit;
		}
		else if(cust_ser == 0)
		{
			retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,FW_IMAGE_NAME_V0, sizeof(FW_IMAGE_NAME_V0),sizeof(FW_IMAGE_NAME_V0));
			fwu->force_update = true;
		}
		else
		{
		...
		dev_dbg(rmi4_data->pdev->dev.parent,"%s: Requesting firmware image %s\n",__func__, fwu->image_name);

		retval = request_firmware(&fw_entry, fwu->image_name,
				rmi4_data->pdev->dev.parent);
		if (retval != 0) {
			dev_err(rmi4_data->pdev->dev.parent,"%s: Firmware image %s not available\n",__func__, fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}

		dev_dbg(rmi4_data->pdev->dev.parent,"%s: Firmware image size = %d\n",__func__, (unsigned int)fw_entry->size);
	...
	{
		dev_err(rmi4_data->pdev->dev.parent,"%s: Flash size mismatch\n",__func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	{
		dev_err(rmi4_data->pdev->dev.parent,"%s: Bootloader version mismatch\n",__func__);
		retval = -EINVAL;
		goto exit;
	}
	...
	if (fwu->in_bl_mode) 
	{
		...
		dev_info(rmi4_data->pdev->dev.parent,"%s: Device in bootloader mode\n",__func__);
	} 
	else
	{
	...
	if (flash_area != NONE && !fwu->bl_mode_device) 
	{
		...
		retval = secure_memcpy(fwu->cal_data, fwu->cal_data_buf_size,&fwu->read_config_buf[fwu->cal_data_off],fwu->cal_data_size, fwu->cal_data_size);
		if (retval < 0) 
		{
			dev_err(rmi4_data->pdev->dev.parent,"%s: Failed to save calibration data\n",__func__);
			rmi4_data->reset_device(rmi4_data, false);
			goto exit;
		}
	}
