...
static unsigned short  gesture_type_addr = 0x0A ;
static unsigned short  swipe_direction_addr =0x0418;
static unsigned short  unicode_switch_addr =0x041E;
static unsigned short  gesture_control_addr=0x22;
...
#define SWIPE_UP_BIT          (0x48)//(0x08)
#define SWIPE_DOWN_BIT        (0x44)//(0x04)
#define SWIPE_L_BIT           (0x42)//(0x02)
#define SWIPE_R_BIT           (0x41)//(0x01)
 ...
 static int synaptics_rmi4_control_gesture(struct synaptics_rmi4_data*rmi4_data)
{
	...
	printk("%s all = %x \n",__func__,all);
	//all = 1;//for meizu test
	//rmi4_data->gesture_mask[UNICODE_INDEX] = 0xFF; // for meizu test
...
printk("-- gesture_control data[0]= %X data[1]= %X data[2]= %X --\n",data[0],data[1],data[2]);
	 dev_err(&i2c->dev, "turn on/off [%x][%x][%x][%x] gesture control\n",
