...
struct pinctrl_state *eint_as_int, *eint_output0, *eint_output1, *rst_output0, *rst_output1, *en_power1, *en_power0;
struct of_device_id touch_of_match[] = {
	...
	{ .compatible = "mediatek,mt8716-touch", },
	{},
};
...
EXPORT_SYMBOL_GPL(tpd_config_gpio);
...
int tpd_driver_add(struct tpd_driver_t *tpd_drv)
{
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name == NULL) {
			...
			TPD_DMESG("tpd_driver_add: load %s successfully\n",tpd_driver_list[i].tpd_device_name);
#if 0
			if (tpd_drv->tpd_local_init() == 0) {
				pr_err("load %s successfully\n",tpd_driver_list[i].tpd_device_name);
...
static int tpd_probe(struct platform_device *pdev)
{
	...
	if (1600 == TPD_RES_Y)
		TPD_RES_Y = 1536;
	pr_err("mtk_tpd: TPD_RES_X = %lu, TPD_RES_Y = %lu\n", TPD_RES_X, TPD_RES_Y);
