...
static unsigned char CTPM_FW[] = {
	#include "M95M_FT8716_SHARP_FW_2016_app.i"
};

static unsigned char aucFW_PRAM_BOOT[] = {
	#include "M95M_FT8716_Pramboot_2016.i"
};

 struct fts_Upgrade_Info fts_updateinfo[] =
{
	{0x87,FTS_MAX_POINTS_10,AUTO_CLB_NONEED,2, 2, 0x87, 0xA6, 20, 2000},//"FT8716"
	{0xEF,FTS_MAX_POINTS_10,AUTO_CLB_NONEED,2, 2, 0x87, 0xA6, 20, 2000}, //,"FT3x07"
...
int  fts_8716_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		fts_i2c_write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		TPD_DMESG("==write fw package %d , sum %d \n", j, packet_number);
		for(i = 0;i < 10; i++)
		{
			auc_i2c_write_buf[0] = 0x6a;
			reg_val[0] = reg_val[1] = 0x00;
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0xEF))
	{
	    	/*FW upgrade*/
    		pbt_buf = CTPM_FW;
	   	 /*call the upgrade function*/
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if(uc_chip_id == fts_updateinfo_curr.CHIP_ID)//call fts_get_upgrade_array in probe function firstly.
	{
		...
		TPD_DMESG("[print1] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
		if (uc_tp_fm_ver == 0xEF || uc_tp_fm_ver < uc_host_fm_ver )
		{
			...
			msleep(100);
			TPD_DMESG("[print2] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
			...
			{
				TPD_DMESG("[FTS] upgrade failed ret=%d.\n", i_ret);
				return -EIO;
			}
		}
	}
