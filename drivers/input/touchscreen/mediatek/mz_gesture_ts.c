...
int mz_gesture_report(struct input_dev *dev, u8 gesture)
{
	...
	result = (1 << (pos - 1)) & (mgs.gesture_mask);
	pr_debug("dump gesture %02x, mask %x, pos %d, result %d \n", gesture, mgs.gesture_mask, pos, result);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(0 == retval) {
		mgs.handle(MZ_GESTURE_DISABLE);
		mgs.gesture_value = retval;
	} else {
		mgs.handle(MZ_GESTURE_ENABLE);
		mgs.gesture_value = retval;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	byte0 = data & 0xff;
	byte2 = (data >> 16) & 0xff;
	pr_err("dump control byte2 %02x, byte0 %02x\n", byte2, byte0);
