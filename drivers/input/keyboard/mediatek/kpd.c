...
#ifdef MEIZU_M80 /* 2015-10-23 Add by BSP@YinShunQing for HALL driver */
int meizu_hall_state = 3; /* 2 : closed  3 : opened */
static int hall_irq;
static int hall_eint_state;
static void hall_key_handler(unsigned long data);
static DECLARE_TASKLET(hall_tasklet, hall_key_handler, 0);
...
static void hall_key_handler(unsigned long data)
{
	if (hall_eint_state == 0) {
		...
		hall_eint_state = 1;
		meizu_hall_state = 2; /* lid shut */
	} else {
		irq_set_irq_type(hall_irq, IRQ_TYPE_LEVEL_LOW);
		hall_eint_state = 0;
		meizu_hall_state = 3; /* lid open */
	}
	kpd_print("[HALL] hall_key_handler %d\n", hall_eint_state);

	input_report_switch(kpd_input_dev, SW_LID, hall_eint_state);
	input_sync(kpd_input_dev);
...
void mz_hall_eint_register(void)
{
	int halls[2] = {0};
	int result = 0;
	...
	hall_node = of_find_compatible_node(NULL, NULL, "mediatek,hall_int-eint");
	if (!hall_node)
		pr_err("[HALL] can't find compatible node\n");
	else {
		of_property_read_u32_array(hall_node, "debounce", halls, ARRAY_SIZE(halls));
		gpio_set_debounce(halls[0], halls[1]);

		hall_irq = irq_of_parse_and_map(hall_node, 0);
		kpd_print("[HALL] mt_eint_register hall_irq=%d\n", hall_irq);
		result = request_irq(hall_irq, hall_eint_handler,
				  IRQF_TRIGGER_NONE, "hall_int-eint", NULL);
		if (result > 0)
			pr_err("[HALL] EINT IRQ line not available!\n");
	}
}
#endif /* end #ifdef MEIZU_M80 */
/*********************************************************************/
...
#ifdef MEIZU_M80 /* 2015-10-23 Add by BSP@YinShunQing for HALL driver */
	wake_lock_init(&hall_wakelock, WAKE_LOCK_SUSPEND, "hall suspend wakelock");
#endif /* end #ifdef MEIZU_M80 */
